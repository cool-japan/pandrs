...
fn test_multi_index_groupby() -> Result<()> {
    ...
    df.add_string_column("category", categories.iter().map(|s| s.to_string()).collect())?;
    ...
    let result_with_multi_index = df.group_by_with_options(["category", "region"], true)?
        .aggregate(vec![
            ("value".to_string(), AggregateOp::Sum, "value_sum".to_string()),
        ])?;
    ...
    assert!(result_with_multi_index.column_names.contains(&"value_sum".to_string()));
    
    // Compare with regular groupby (without multi-index)
    let result_without_multi_index = df.group_by_with_options(["category", "region"], false)?
        .aggregate(vec![
            ("value".to_string(), AggregateOp::Sum, "value_sum".to_string()),
        ])?;
    ...
    assert!(result_without_multi_index.column_names.contains(&"category".to_string()));
    assert!(result_without_multi_index.column_names.contains(&"region".to_string()));
    assert!(result_without_multi_index.column_names.contains(&"value_sum".to_string()));
    ...
    let par_result = df.group_by_with_options(["category", "region"], true)?
        .par_aggregate(vec![
            ("value".to_string(), AggregateOp::Sum, "value_sum".to_string()),
        ])?;
    ...
    let custom_result = df.group_by_with_options(["category", "region"], true)?
    ...
    assert!(custom_result.column_names.contains(&"value_range".to_string()));
    
    // Test parallel custom aggregation
    let par_custom_result = df.group_by_with_options(["category", "region"], true)?
    ...
    assert!(par_custom_result.column_names.contains(&"value_range".to_string()));
...
fn test_single_column_groupby_multi_index_flag() -> Result<()> {
    ...
    let result_with_flag = df.group_by_with_options(["category"], true)?
        .aggregate(vec![
            ("value".to_string(), AggregateOp::Sum, "value_sum".to_string()),
        ])?;
    
    // Compare with regular groupby
    let result_without_flag = df.group_by_with_options(["category"], false)?
        .aggregate(vec![
            ("value".to_string(), AggregateOp::Sum, "value_sum".to_string()),
        ])?;
    ...
    assert_eq!(result_with_flag.column_names.len(), result_without_flag.column_names.len());
