...
mod parquet_error_tests {
    ...
    fn test_parquet_write_invalid_path() {
        ...
        let result = write_parquet(&df, "/nonexistent_directory/test.parquet", Some(ParquetCompression::Snappy));
        assert!(result.is_err());
    }
    ...
    fn test_parquet_compression_options() {
        let mut df = OptimizedDataFrame::new();
        df.add_int_column("test", vec![1, 2, 3]).unwrap();
        df.add_string_column("text", vec!["a".to_string(), "b".to_string(), "c".to_string()]).unwrap();
        ...
        for compression in compression_options.iter() {
            let temp_path = std::env::temp_dir().join(format!("compression_test_{:?}.parquet", compression));
...
mod sql_error_tests {
    use super::*;
    use pandrs::io::{read_sql, execute_sql, write_to_sql};
