...
mod empty_data_tests {
    ...
    fn test_empty_series_operations() {
        ...
        let empty_int_series: Series<i64> = Series::new(vec![], Some("empty_int".to_string())).unwrap();
        assert_eq!(empty_int_series.len(), 0);
        assert!(empty_int_series.is_empty());

        // Test empty float series
        let empty_float_series: Series<f64> = Series::new(vec![], Some("empty_float".to_string())).unwrap();
        assert_eq!(empty_float_series.len(), 0);

        // Test empty string series
        let empty_string_series: Series<String> = Series::new(vec![], Some("empty_string".to_string())).unwrap();
...
mod boundary_condition_tests {
    ...
    fn test_extreme_numeric_values() {
        ...
        df.add_int_column("extreme_int", vec![i64::MAX, i64::MIN, 0]).unwrap();
        df.add_float_column("extreme_float", vec![f64::MAX, f64::MIN, 0.0]).unwrap();
    ...
    fn test_very_long_strings() {
        ...
        let mut df = OptimizedDataFrame::new();
        df.add_string_column("long_strings", vec![
            long_string.clone(),
    ...
    fn test_nan_and_infinity_handling() {
        ...
        df.add_float_column("special_values", vec![
            f64::NAN,
            f64::INFINITY,
            f64::NEG_INFINITY,
            0.0,
...
mod resource_management_tests {
    ...
    fn test_string_pool_stress() {
        ...
        df.add_string_column("stress_test", string_data.clone()).unwrap();
...
mod type_system_tests {
    ...
    fn test_mixed_type_operations() {
        ...
        df.add_string_column("strings", vec!["a".to_string(), "b".to_string(), "c".to_string()]).unwrap();
    ...
    fn test_series_name_operations() {
        ...
        // Test with_name
        let named_series = Series::new(vec![4, 5, 6], None).unwrap()
...
mod dataframe_creation_tests {
    ...
    fn test_from_map_mismatched_lengths() {
        ...
        data.insert("col2".to_string(), vec!["x".to_string(), "y".to_string(), "z".to_string()]);
        ...
        match result {
            Ok(df) => {
                ...
                println!("DataFrame created with {} rows, {} columns", df.row_count(), df.column_count());
            }
            Err(e) => {
    ...
    fn test_from_map_invalid_column_names() {
        ...
        match result {
            Ok(df) => {
                ...
                println!("DataFrame created with invalid column names: {} rows, {} columns", df.row_count(), df.column_count());
            }
            Err(e) => {
