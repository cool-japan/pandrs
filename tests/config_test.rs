...
fn test_config_file_operations() {
    ...
    assert_eq!(loaded_config.database.pool.max_connections, original_config.database.pool.max_connections);
    assert_eq!(loaded_config.cloud.global.timeout, original_config.cloud.global.timeout);
...
fn test_credential_builder_patterns() {
    ...
    let api_cred = CredentialBuilder::new()
        .api_key("sk-1234567890abcdef", Some("secret"), Some("https://api.example.com"))
...
fn test_configuration_integration() {
    ...
    let db_cred = CredentialBuilder::new()
        .database("config_user", "config_pass", "config.db.com", 5432, "config_db")
        .build()
        .unwrap();
...
fn test_config_file_discovery() {
    ...
    assert!(paths.iter().any(|p| p.file_name().unwrap() == "pandrs.yaml"));
    assert!(paths.iter().any(|p| p.file_name().unwrap() == "pandrs.toml"));
...
fn test_error_handling() {
    ...
    let result = store_mut.store_credential("test", credential);
