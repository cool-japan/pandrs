...
fn test_alpha4_dataframe_operations() -> Result<()> {
    ...
    assert_eq!(df.column_names(), vec!["employee_name", "employee_age", "salary"]);
    ...
    assert_eq!(df.column_names(), vec!["worker_name", "worker_age", "worker_salary"]);
...
fn test_alpha4_optimized_dataframe_operations() -> Result<()> {
    ...
    assert_eq!(df.column_names(), vec!["emp_name", "emp_age", "emp_salary", "emp_active"]);
...
fn test_alpha4_distributed_processing_integration() -> Result<()> {
    ...
    df.add_column(
        "sales".to_string(),
        pandrs::series::Series::from_vec(vec![1000, 1500, 800, 1200, 900, 1100], Some("sales".to_string()))
    )?;
...
fn test_alpha4_cross_feature_integration() -> Result<()> {
    ...
    let dept_summary = emp_df
        .aggregate(&["department"], &[("salary", "avg", "avg_salary"), ("salary", "count", "emp_count")])?
        .collect()?;
