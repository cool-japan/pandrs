...
fn test_dataframe_rename_columns() {
    ...
    let series1 = Series::new(vec!["a".to_string(), "b".to_string()], Some("col1".to_string())).unwrap();
    let series2 = Series::new(vec!["c".to_string(), "d".to_string()], Some("col2".to_string())).unwrap();
    ...
    assert_eq!(column_names, vec!["new_col1".to_string(), "new_col2".to_string()]);
...
fn test_dataframe_set_column_names() {
    ...
    let column_names = df.column_names();
    assert_eq!(column_names, vec!["first".to_string(), "second".to_string()]);
...
fn test_series_set_name() {
    let mut series = Series::new(vec!["a".to_string(), "b".to_string()], Some("original".to_string())).unwrap();
...
fn test_series_with_name() {
    let series = Series::new(vec!["a".to_string(), "b".to_string()], Some("original".to_string())).unwrap();
