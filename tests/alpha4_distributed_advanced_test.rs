...
mod tests {
    use pandrs::distributed::{DistributedContext, expr::{Expr, ColumnProjection, ExprSchema, ExprDataType, ExprColumn}};
    ...
    fn test_alpha4_schema_validation() -> Result<()> {
        ...
        let invalid_projections = vec![
            ColumnProjection::column("nonexistent_column"),
        ];
    ...
    fn test_alpha4_complex_distributed_queries() -> Result<()> {
        ...
        let emp_names = vec!["Alice", "Bob", "Carol", "David", "Eve", "Frank", "Grace", "Henry"];
        
        for i in 0..100 {
            departments.push(dept_names[i % dept_names.len()].to_string());
            employees.push(format!("{}{}", emp_names[i % emp_names.len()], i / emp_names.len()));
    ...
    fn test_alpha4_distributed_window_operations() -> Result<()> {
        ...
        let category_summary = ts_df
            .aggregate(&["category"], &[("value", "sum", "total_value"), ("value", "avg", "avg_value")])?
            .collect()?;
    ...
    fn test_alpha4_distributed_performance() -> Result<()> {
        ...
        for concurrency in [1, 2, 4] {
            ...
            // Perform aggregation
            let result = large_df
                .aggregate(&["group"], &[("value", "sum", "total"), ("id", "count", "count")])?
                .collect()?;
