...
use crate::storage::unified_manager::{StrategySelector, StrategySelection, PerformanceMonitor, StrategyMetrics};
...
impl LinearRegressionModel {
    ...
    pub fn confidence(&self) -> f64 {
        if self.training_count < 10 {
            ...
            (1.0 - self.accuracy_metrics.mean_absolute_error()).max(0.1).min(0.95)
        }
    }
}
...
impl MLStrategySelector {
    ...
    pub fn predict_performance(&self, strategy: StorageType, features: &WorkloadFeatures) -> PerformancePrediction {
        let feature_vector = features.to_vector();
        
        let throughput = self.throughput_models.get(&strategy)
            .map(|model| model.predict(&feature_vector))
            .unwrap_or(1000000.0); // Default throughput
            
        let latency = self.latency_models.get(&strategy)
            .map(|model| model.predict(&feature_vector))
            .unwrap_or(10.0); // Default latency
            
        let memory_usage = self.memory_models.get(&strategy)
            .map(|model| model.predict(&feature_vector))
            .unwrap_or(features.data_size * 1.2); // Default memory overhead
            
        let cpu_usage = self.cpu_models.get(&strategy)
            .map(|model| model.predict(&feature_vector))
            .unwrap_or(20.0); // Default CPU usage
            
        let confidence = self.throughput_models.get(&strategy)
    ...
    pub fn select_best_strategy(&self, requirements: &StorageRequirements) -> StrategySelection {
        ...
        let confidence = strategy_scores.first()
            .map(|(_, _, conf)| *conf)
            .unwrap_or(0.1);
    ...
    fn calculate_strategy_score(&self, prediction: &PerformancePrediction, requirements: &StorageRequirements) -> f64 {
        let mut score = 0.0;
        ...
        let memory_efficiency = requirements.estimated_size as f64 / prediction.memory_usage.max(1.0);
    ...
    pub fn add_training_example(&mut self, example: TrainingExample) {
        ...
        if self.training_data.len() > self.max_training_data {
            self.training_data.drain(0..self.training_data.len() - self.max_training_data);
        }
        ...
        if let Some(model) = self.throughput_models.get_mut(&example.strategy) {
            model.train(&features, example.performance.throughput, self.learning_rate);
        }
        ...
        if let Some(model) = self.memory_models.get_mut(&example.strategy) {
            model.train(&features, example.performance.memory_usage, self.learning_rate);
        }
    ...
    pub fn get_model_stats(&self) -> HashMap<StorageType, ModelStats> {
        ...
        for (&strategy, model) in &self.throughput_models {
            stats.insert(strategy, ModelStats {
...
impl StrategySelector for MLStrategySelector {
    ...
    fn record_performance(&mut self, strategy_type: StorageType, performance: &StrategyMetrics) {
        ...
        if let Some(read_time) = performance.average_operation_time(crate::storage::unified_manager::OperationType::Read) {
            if let Some(write_time) = performance.average_operation_time(crate::storage::unified_manager::OperationType::Write) {
                let total_time = read_time + write_time;
                if total_time.as_nanos() > 0 {
                    features.read_write_ratio = read_time.as_nanos() as f64 / total_time.as_nanos() as f64;
                }
            }
        }
        ...
        let prediction = PerformancePrediction {
            throughput: performance.throughput(crate::storage::unified_manager::OperationType::Read).unwrap_or(0.0),
            latency: performance.average_operation_time(crate::storage::unified_manager::OperationType::Read)
                .unwrap_or(Duration::from_millis(1)).as_millis() as f64,
...
impl AdaptiveUnifiedMemoryManager {
    pub fn new(config: crate::storage::unified_manager::MemoryConfig) -> Self {
        ...
        let ml_selector = Arc::new(Mutex::new(MLStrategySelector::new(Arc::clone(&performance_monitor))));
    ...
    pub fn adapt(&mut self) -> Result<()> {
        ...
        if let Ok(mut selector) = self.ml_selector.lock() {
            ...
            for (strategy, stat) in stats {
                println!("Strategy {:?}: {} examples, {:.2} confidence, {:.2} accuracy", 
    ...
    pub fn create_storage_ml(&mut self, config: &StorageConfig) -> Result<StorageHandle> {
        ...
        if let Ok(selector) = self.ml_selector.lock() {
            ...
            println!("ML selected strategy: {:?} with confidence {:.2}", 
