...
impl PerformanceMonitor {
    ...
    pub fn record_operation(
        ...
        self.system_metrics.record_operation(operation, duration, bytes);
    }
...
impl StrategyMetrics {
    ...
    fn record_operation(&mut self, operation: OperationType, duration: std::time::Duration, bytes: usize) {
        *self.operation_counts.entry(operation).or_insert(0) += 1;
        *self.operation_times.entry(operation).or_insert(std::time::Duration::ZERO) += duration;
...
impl SystemMetrics {
    ...
    fn record_operation(&mut self, _operation: OperationType, duration: std::time::Duration, bytes: usize) {
...
pub struct UnifiedMemoryManager {
    ...
    strategies: HashMap<StorageType, Box<dyn StorageStrategy<Handle = StorageHandle, Error = Error, Metadata = StorageMetadata>>>,
...
impl UnifiedMemoryManager {
    ...
    pub fn create_storage(&mut self, config: &StorageConfig) -> Result<StorageHandle> {
            ...
            match strategy.create_storage(config) {
                Ok(handle) => {
                    let storage_id = StorageId(
                        self.next_storage_id.fetch_add(1, std::sync::atomic::Ordering::SeqCst)
                    );
        ...
        Err(Error::InvalidOperation("No suitable storage strategy available".to_string()))
    }
    ...
    pub fn read_chunk(&self, handle: &StorageHandle, range: ChunkRange) -> Result<DataChunk> {
        ...
        if let Some(strategy) = self.strategies.get(&handle.strategy_type) {
            ...
            Err(Error::InvalidOperation(format!("Strategy {:?} not found", handle.strategy_type)))
        }
    }
    ...
    pub fn add_strategy(
        &mut self,
        strategy_type: StorageType,
        strategy: Box<dyn StorageStrategy<Handle = StorageHandle, Error = Error, Metadata = StorageMetadata>>,
    ) {
...
impl StrategySelector for DefaultStrategySelector {
    fn select_strategy(&self, requirements: &StorageRequirements) -> StrategySelection {
        ...
        let primary = match (&requirements.data_characteristics, requirements.estimated_size) {
    ...
    fn record_performance(&mut self, strategy_type: StorageType, _performance: &StrategyMetrics) {
        ...
        self.performance_history.entry(strategy_type).or_insert_with(Vec::new);
    }
}
