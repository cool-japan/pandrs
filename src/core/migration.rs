...
impl Version {
    ...
    pub fn parse(version_str: &str) -> Result<Self> {
        ...
        if version_nums.len() != 3 {
            return Err(Error::InvalidInput(format!("Invalid version format: {}", version_str)));
        }
        ...
        let patch = version_nums[2].parse::<u32>()
...
impl BackupManager {
    ...
    pub fn create_backup(&mut self, data_path: &Path) -> Result<String> {
        let backup_id = format!("backup_{}", SystemTime::now().duration_since(SystemTime::UNIX_EPOCH)
        ...
        self.metadata.insert(backup_id.clone(), backup_dir.to_string_lossy().to_string());
    ...
    pub fn restore_backup(&self, backup_id: &str, target_path: &Path) -> Result<()> {
        if let Some(backup_path) = self.metadata.get(backup_id) {
            ...
            if backup_dir.exists() {
                ...
                Err(Error::IoError(format!("Backup directory not found: {}", backup_path)))
            }
        } else {
            Err(Error::InvalidInput(format!("Backup ID not found: {}", backup_id)))
        }
    }
    ...
    pub fn execute(&mut self) -> Result<MigrationResult> {
        ...
        for (index, step) in steps.iter().enumerate() {
            ...
            match self.execute_step(step) {
                Ok(result) => {
                    self.context.progress.complete_step(step.id.clone(), total_steps);
                    step_results.push(result);
                }
                Err(error) => {
                    self.context.progress.fail_step(step.id.clone(), error.to_string());
...
impl MigrationPlanBuilder {
    ...
    pub fn build(self) -> Result<MigrationPlan> {
        let from_version = self.from_version.ok_or_else(|| 
            Error::InvalidInput("From version not specified".to_string()))?;
        let to_version = self.to_version.ok_or_else(|| 
            Error::InvalidInput("To version not specified".to_string()))?;
        
        let estimated_duration = self.steps.iter()
