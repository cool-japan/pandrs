...
pub fn load_from_env() -> Result<PandRSConfig> {
    ...
    if let Ok(pool_size) = env::var("PANDRS_DB_POOL_SIZE") {
        config.database.pool.max_connections = pool_size.parse()
            .map_err(|e| Error::ConfigurationError(format!("Invalid PANDRS_DB_POOL_SIZE: {}", e)))?;
    }
    
    if let Ok(timeout) = env::var("PANDRS_DB_TIMEOUT") {
        config.database.timeouts.query = timeout.parse()
    ...
    if let Ok(threads) = env::var("PANDRS_WORKER_THREADS") {
        config.performance.threading.worker_threads = threads.parse()
            .map_err(|e| Error::ConfigurationError(format!("Invalid PANDRS_WORKER_THREADS: {}", e)))?;
    }
    
    if let Ok(parallel) = env::var("PANDRS_PARALLEL_ENABLED") {
        config.performance.threading.parallel_enabled = parallel.parse()
            .map_err(|e| Error::ConfigurationError(format!("Invalid PANDRS_PARALLEL_ENABLED: {}", e)))?;
    }
    
    if let Ok(memory_limit) = env::var("PANDRS_MEMORY_LIMIT") {
        config.performance.memory.limit = memory_limit.parse()
            .map_err(|e| Error::ConfigurationError(format!("Invalid PANDRS_MEMORY_LIMIT: {}", e)))?;
    }
    
    if let Ok(jit_enabled) = env::var("PANDRS_JIT_ENABLED") {
        config.performance.jit.enabled = jit_enabled.parse()
    ...
    if let Ok(encryption) = env::var("PANDRS_ENCRYPTION_ENABLED") {
        config.security.encryption.enabled = encryption.parse()
            .map_err(|e| Error::ConfigurationError(format!("Invalid PANDRS_ENCRYPTION_ENABLED: {}", e)))?;
    }
    
    if let Ok(audit) = env::var("PANDRS_AUDIT_ENABLED") {
        config.security.audit.enabled = audit.parse()
            .map_err(|e| Error::ConfigurationError(format!("Invalid PANDRS_AUDIT_ENABLED: {}", e)))?;
    }
...
pub fn load_from_file(path: &Path) -> Result<PandRSConfig> {
    if !path.exists() {
        return Err(Error::ConfigurationError(format!("Configuration file not found: {}", path.display())));
    }
    
    let contents = fs::read_to_string(path)
        .map_err(|e| Error::ConfigurationError(format!("Failed to read config file {}: {}", path.display(), e)))?;
    
    match path.extension().and_then(|ext| ext.to_str()) {
        Some("yaml") | Some("yml") => load_from_yaml(&contents),
        Some("toml") => load_from_toml(&contents),
        Some(ext) => Err(Error::ConfigurationError(format!("Unsupported config file format: {}", ext))),
        None => {
...
pub fn save_to_file(config: &PandRSConfig, path: &Path) -> Result<()> {
    let contents = match path.extension().and_then(|ext| ext.to_str()) {
        ...
        Some(ext) => return Err(Error::ConfigurationError(format!("Unsupported config file format: {}", ext))),
    ...
    if let Some(parent) = path.parent() {
        if !parent.exists() {
            fs::create_dir_all(parent)
                .map_err(|e| Error::ConfigurationError(format!("Failed to create config directory {}: {}", parent.display(), e)))?;
        }
    }
    
    fs::write(path, contents)
        .map_err(|e| Error::ConfigurationError(format!("Failed to write config file {}: {}", path.display(), e)))
}
...
mod tests {
    ...
    fn test_load_from_env() {
        ...
        assert_eq!(config.database.default_url, Some("postgresql://test:test@localhost/test".to_string()));
...
"#;
        ...
        assert_eq!(config.database.default_url, Some("postgresql://localhost/test".to_string()));
    ...
    fn test_save_and_load_file() {
        ...
        assert_eq!(original_config.to_yaml().unwrap(), loaded_config.to_yaml().unwrap());
    }
