...
impl<'a> PivotTable<'a> {
    ...
    pub fn new(
        ...
        if !df.contains_column(&index) {
            return Err(PandRSError::Column(format!("インデックス列 '{}' が見つかりません", index)));
        }
        if !df.contains_column(&columns) {
            return Err(PandRSError::Column(format!("カラム列 '{}' が見つかりません", columns)));
        }
        if !df.contains_column(&values) {
            return Err(PandRSError::Column(format!("値列 '{}' が見つかりません", values)));
        }
    ...
    pub fn execute(&self) -> Result<DataFrame> {
        ...
        for i in 0..self.df.row_count() {
            if i < index_values_vec.len() && i < column_values_vec.len() && i < values_data_vec.len() {
                ...
                aggregation_map.entry(key).or_insert_with(Vec::new).push(value);
            }
        }
    ...
    fn aggregate_values_from_vec(&self, values: &[f64]) -> Result<f64> {
        ...
        match self.aggfunc {
            ...
            AggFunction::Min => {
                if let Some(min) = values.iter().fold(None, |min, &x| {
                    match min {
                        None => Some(x),
            ...
            AggFunction::Max => {
                if let Some(max) = values.iter().fold(None, |max, &x| {
                    match max {
                        None => Some(x),
                        Some(y) => Some(if x > y { x } else { y })
                    }
...
impl DataFrame {
    ...
    pub fn groupby(&self, by: &str) -> Result<GroupBy> {
        if !self.contains_column(by) {
            return Err(PandRSError::Column(format!("グループ化列 '{}' が見つかりません", by)));
        }
    ...
    pub fn agg(&self, columns: &[&str], aggfunc: AggFunction) -> Result<DataFrame> {
        ...
        for col in columns {
            if !self.df.contains_column(col) {
                return Err(PandRSError::Column(format!("集計列 '{}' が見つかりません", col)));
            }
        }
