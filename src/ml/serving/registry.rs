...
use crate::ml::serving::{ModelMetadata, ModelServing, SerializationFormat, ModelSerializer};
use crate::ml::serving::serialization::{
    SerializableModel, ModelSerializationFactory, JsonModelSerializer, 
    YamlModelSerializer, TomlModelSerializer, BinaryModelSerializer
};
...
pub trait ModelRegistry {
    ...
    fn update_metadata(&mut self, name: &str, version: &str, metadata: ModelMetadata) -> Result<()>;
...
impl InMemoryModelRegistry {
    ...
    fn update_entry(&mut self, name: &str, version: &str, metadata: &ModelMetadata) {
        ...
        if entry.default_version.is_none() {
            ...
            self.default_versions.insert(name.to_string(), version.to_string());
        }
...
impl ModelRegistry for InMemoryModelRegistry {
    fn register_model(&mut self, model: Box<dyn ModelServing>) -> Result<()> {
        ...
        if self.exists(&name, &version) {
            return Err(Error::InvalidOperation(format!(
                "Model '{}' version '{}' already exists", name, version
            )));
        }
    ...
    fn load_model(&self, name: &str, version: &str) -> Result<Box<dyn ModelServing>> {
        ...
        self.models
            ...
            .ok_or_else(|| Error::KeyNotFound(format!(
                "Model '{}' version '{}' not found", name, resolved_version
            )))
            .map(|_| {
        ...
        if let Some(versions) = self.models.get_mut(name) {
            if versions.remove(version).is_some() {
                ...
                if let Some(entry) = self.entries.get_mut(name) {
                    ...
                    if entry.default_version.as_ref() == Some(&version.to_string()) {
                        ...
                        if let Some(new_default) = &entry.default_version {
                            self.default_versions.insert(name.to_string(), new_default.clone());
                        } else {
                ...
                Err(Error::KeyNotFound(format!(
                    "Model '{}' version '{}' not found", name, version
                )))
            }
        } else {
...
impl ModelRegistry for FileSystemModelRegistry {
    fn register_model(&mut self, model: Box<dyn ModelServing>) -> Result<()> {
        ...
        ModelSerializationFactory::save_model(&serializable_model, &model_file, self.default_format)?;
    ...
    fn update_metadata(&mut self, name: &str, version: &str, new_metadata: ModelMetadata) -> Result<()> {
        let model_file = self.get_model_file(name, version);
