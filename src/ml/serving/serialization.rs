...
use crate::ml::serving::{ModelMetadata, ModelServing, ModelInfo, HealthStatus, ModelStatistics};
use crate::ml::serving::{PredictionRequest, PredictionResponse, BatchPredictionRequest, BatchPredictionResponse};
...
impl ModelSerializer for JsonModelSerializer {
    ...
    fn load<P: AsRef<Path>>(&self, path: P) -> Result<Box<dyn ModelServing>> {
        ...
        Ok(Box::new(GenericServingModel::from_serializable(serializable_model)?))
    }
...
impl ModelSerializer for YamlModelSerializer {
    ...
    fn load<P: AsRef<Path>>(&self, path: P) -> Result<Box<dyn ModelServing>> {
        ...
        let serializable_model: SerializableModel = serde_yaml::from_str(&yaml_data)
            .map_err(|e| Error::SerializationError(format!("YAML deserialization failed: {}", e)))?;
    ...
    fn deserialize(&self, data: &[u8]) -> Result<SerializableModel> {
        ...
        let model = serde_yaml::from_str(yaml_str)
            .map_err(|e| Error::SerializationError(format!("YAML deserialization failed: {}", e)))?;
        Ok(model)
    }
...
impl ModelSerializer for TomlModelSerializer {
    ...
    fn load<P: AsRef<Path>>(&self, path: P) -> Result<Box<dyn ModelServing>> {
        ...
        let serializable_model: SerializableModel = toml::from_str(&toml_data)
            .map_err(|e| Error::SerializationError(format!("TOML deserialization failed: {}", e)))?;
    ...
    fn deserialize(&self, data: &[u8]) -> Result<SerializableModel> {
        let toml_str = std::str::from_utf8(data)
            .map_err(|e| Error::SerializationError(format!("Invalid UTF-8: {}", e)))?;
        let model = toml::from_str(toml_str)
            .map_err(|e| Error::SerializationError(format!("TOML deserialization failed: {}", e)))?;
        Ok(model)
    }
...
impl GenericServingModel {
    ...
    fn perform_prediction(&self, _input_data: &HashMap<String, serde_json::Value>) -> Result<serde_json::Value> {
...
impl ModelSerializationFactory {
    ...
    pub fn save_model<P: AsRef<Path>>(model: &SerializableModel, path: P, format: SerializationFormat) -> Result<()> {
        match format {
            SerializationFormat::Json => {
    ...
    pub fn load_model<P: AsRef<Path>>(path: P, format: SerializationFormat) -> Result<Box<dyn ModelServing>> {
        match format {
            SerializationFormat::Json => {
                let serializer = JsonModelSerializer;
    ...
    pub fn auto_detect_and_load<P: AsRef<Path>>(path: P) -> Result<Box<dyn ModelServing>> {
        ...
        let format = SerializationFormat::from_extension(extension)
            .ok_or_else(|| Error::InvalidInput(format!("Unsupported file extension: {}", extension)))?;
