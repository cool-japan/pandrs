...
impl Transformer for KMeans {
    ...
    fn transform(&self, df: &DataFrame) -> Result<DataFrame> {
        ...
        for point in &data {
            ...
            labels.push(DataValue::Int64(closest_centroid as i64));
            distances.push(DataValue::Float64(min_dist.sqrt())); // ユークリッド距離
        }
        
        // クラスタラベルと距離を新しい列として追加
        let labels_series = Series::from_vec(labels)?;
        let distances_series = Series::from_vec(distances)?;
        
        result.add_column("cluster".to_string(), labels_series)?;
        result.add_column("distance_to_centroid".to_string(), distances_series)?;
...
impl Transformer for AgglomerativeClustering {
    ...
    fn transform(&self, df: &DataFrame) -> Result<DataFrame> {
        ...
        // クラスタラベルを新しい列として追加
        let labels: Vec<DataValue> = self.labels.iter().map(|&l| DataValue::Int64(l as i64)).collect();
        let labels_series = Series::from_vec(labels)?;
...
impl Transformer for DBSCAN {
    ...
    fn transform(&self, df: &DataFrame) -> Result<DataFrame> {
        ...
        // クラスタラベルを新しい列として追加
        let labels: Vec<DataValue> = self.labels.iter().map(|&l| DataValue::Int64(l)).collect();
        let labels_series = Series::from_vec(labels)?;
