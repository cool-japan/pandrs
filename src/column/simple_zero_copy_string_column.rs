...
use crate::storage::simple_unified_string_pool::{SimpleUnifiedStringPool, SimpleStringView, SimpleStringPoolStats};
...
impl SimpleZeroCopyStringColumn {
    ...
    pub fn to_lowercase_optimized(&self) -> Result<SimpleZeroCopyStringColumn> {
        let new_strings = self.map_views(|view_opt| {
            match view_opt {
                Some(view) => view.as_str().unwrap_or_else(|_| String::new()).to_lowercase(),
                None => String::new(),
            }
        })?;
    ...
    pub fn to_uppercase_optimized(&self) -> Result<SimpleZeroCopyStringColumn> {
        let new_strings = self.map_views(|view_opt| {
            match view_opt {
                Some(view) => view.as_str().unwrap_or_else(|_| String::new()).to_uppercase(),
                None => String::new(),
            }
        })?;
    ...
    pub fn concat_with(&self, other: &SimpleZeroCopyStringColumn, separator: &str) -> Result<SimpleZeroCopyStringColumn> {
...
mod tests {
    ...
    fn test_string_deduplication() {
        ...
        assert!(stats.deduplication_ratio > 0.0, "Deduplication ratio: {}", stats.deduplication_ratio);
        assert!((stats.deduplication_ratio - 0.4).abs() < 0.001, "Expected ~0.4, got {}", stats.deduplication_ratio);
    ...
    fn test_shared_pool() {
        ...
        let column1 = SimpleZeroCopyStringColumn::with_shared_pool(data1, Arc::clone(&pool)).unwrap();
        let column2 = SimpleZeroCopyStringColumn::with_shared_pool(data2, Arc::clone(&pool)).unwrap();
