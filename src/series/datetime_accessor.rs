...
impl DateTimeAccessor {
        ...
        let rounded: Vec<NaiveDateTime> = self.series.values()
            ...
            .map(|dt| {
                match freq {
                    ...
                    "S" | "second" => dt.date().and_hms_opt(dt.hour(), dt.minute(), dt.second()).unwrap_or(*dt),
                    ...
                    freq_str if freq_str.ends_with("min") => {
                        if let Ok(minutes) = freq_str.trim_end_matches("min").parse::<u32>() {
                            ...
                            dt.date().and_hms_opt(dt.hour(), rounded_minute, 0).unwrap_or(*dt)
                        } else {
                            *dt
                        }
                    },
                    ...
                    freq_str if freq_str.ends_with("S") => {
                        if let Ok(seconds) = freq_str.trim_end_matches("S").parse::<u32>() {
                            ...
                            dt.date().and_hms_opt(dt.hour(), dt.minute(), rounded_second).unwrap_or(*dt)
                        } else {
                            *dt
                        }
                    },
    ...
    pub fn week(&self) -> Result<Series<u32>, PandrsError> {
        let weeks: Vec<u32> = self.series.values()
            .iter()
    ...
    pub fn is_leap_year(&self) -> Result<Series<bool>, PandrsError> {
        let is_leap: Vec<bool> = self.series.values()
            .iter()
    ...
    pub fn is_business_day(&self) -> Result<Series<bool>, PandrsError> {
        let is_bday: Vec<bool> = self.series.values()
            .iter()
            .map(|dt| {
                let weekday = dt.weekday().num_days_from_monday();
        ...
        let new_dates: Vec<NaiveDateTime> = self.series.values()
            ...
            .map(|dt| {
                ...
                if let Some(new_date) = NaiveDate::from_ymd_opt(year, month as u32, day) {
                    new_date.and_hms_opt(dt.hour(), dt.minute(), dt.second()).unwrap_or(*dt)
                } else {
    ...
    pub fn business_day_count(&self, end_date: NaiveDateTime) -> Result<Series<i64>, PandrsError> {
        let counts: Vec<i64> = self.series.values()
            .iter()
            .map(|start_dt| {
                let start = if *start_dt <= end_date { *start_dt } else { end_date };
                let end = if *start_dt <= end_date { end_date } else { *start_dt };
                ...
                if *start_dt > end_date { -count } else { count }
            })
            .collect();
...
mod tests {
    ...
    fn test_enhanced_datetime_features() {
        let dt1 = NaiveDate::from_ymd_opt(2023, 2, 15).unwrap().and_hms_opt(14, 30, 45).unwrap(); // Feb 2023
        let dt2 = NaiveDate::from_ymd_opt(2024, 2, 15).unwrap().and_hms_opt(9, 15, 30).unwrap();  // Feb 2024 (leap year)
        let dt3 = NaiveDate::from_ymd_opt(2023, 12, 23).unwrap().and_hms_opt(10, 0, 0).unwrap();  // Saturday
    ...
    fn test_advanced_date_arithmetic() {
        let dt = NaiveDate::from_ymd_opt(2023, 1, 31).unwrap().and_hms_opt(14, 30, 45).unwrap();
        let data = vec![dt];
        let series = Series::new(data, Some("test_dates".to_string())).unwrap();
        let dt_accessor = DateTimeAccessor::new(series).unwrap();
    ...
    fn test_enhanced_rounding() {
        let dt = NaiveDate::from_ymd_opt(2023, 12, 25).unwrap().and_hms_opt(14, 37, 23).unwrap();
        let data = vec![dt];
        let series = Series::new(data, Some("test_dates".to_string())).unwrap();
        let dt_accessor = DateTimeAccessor::new(series).unwrap();
    ...
    fn test_business_day_count() {
        ...
        let start_dt = NaiveDate::from_ymd_opt(2023, 12, 25).unwrap().and_hms_opt(0, 0, 0).unwrap(); // Monday
        let data = vec![start_dt];
        let series = Series::new(data, Some("test_dates".to_string())).unwrap();
        let dt_accessor = DateTimeAccessor::new(series).unwrap();
        
        let end_dt = NaiveDate::from_ymd_opt(2023, 12, 29).unwrap().and_hms_opt(0, 0, 0).unwrap(); // Friday
    ...
    fn test_leap_year_edge_cases() {
        ...
        let leap_day = NaiveDate::from_ymd_opt(2024, 2, 29).unwrap().and_hms_opt(12, 0, 0).unwrap();
