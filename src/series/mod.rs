    ...
    T: Debug + Clone,
{
        ...
        I: Debug + Clone + Eq + std::hash::Hash + std::fmt::Display,
    {
        ...
        Ok(Series { values, index: range_index, name })
    }
...
impl<T> Series<T>
where
    T: Debug + Clone + Copy + Sum<T> + PartialOrd +
       ...
       NumCast + Default,
{
    ...
    pub fn mean(&self) -> Result<T> {
        if self.values.is_empty() {
            return Err(PandRSError::Consistency("空のSeriesの平均は計算できません".to_string()));
        }
        
        let sum = self.sum();
        let count = match num_traits::cast(self.len()) {
            Some(n) => n,
            None => return Err(PandRSError::Cast(
    ...
    pub fn min(&self) -> Result<T> {
        if self.values.is_empty() {
            return Err(PandRSError::Consistency("空のSeriesの最小値は計算できません".to_string()));
        }
    ...
    pub fn max(&self) -> Result<T> {
        if self.values.is_empty() {
            return Err(PandRSError::Consistency("空のSeriesの最大値は計算できません".to_string()));
        }
