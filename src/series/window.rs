        ...
        R: Debug + Clone,
    {
        ...
        for i in 0..values.len() {
            let (start, end) = if self.center {
                ...
                let start = if i + 1 >= self.window_size { i + 1 - self.window_size } else { 0 };
                let end = i + 1;
                (start, end)
            };
            
            // Extract non-null values in the window
            let window_values: Vec<f64> = values[start..end]
                .iter()
                .filter_map(|&v| v)
                .collect();
    ...
    T: Debug + Clone + Into<f64> + Copy,
{
    fn mean(&self) -> Result<Series<f64>> {
        ...
        let values: Vec<f64> = result.values().iter()
    ...
    fn count(&self) -> Result<Series<usize>> {
        ...
        let values: Vec<usize> = result.values().iter()
            .map(|&v| v.unwrap_or(0))
            .collect();
        Series::new(values, result.name().cloned())
    }
    ...
    fn quantile(&self, q: f64) -> Result<Series<f64>> {
        if q < 0.0 || q > 1.0 {
            return Err(Error::InvalidValue("Quantile must be between 0 and 1".to_string()));
        }
        ...
        R: Debug + Clone,
    {
        ...
        let values: Vec<R> = result.values().iter()
    ...
    T: Debug + Clone,
{
        ...
        R: Debug + Clone,
    {
        ...
        for i in 0..values.len() {
            ...
            let window_values: Vec<f64> = values[0..=i]
                .iter()
                .filter_map(|&v| v)
                .collect();
    ...
    T: Debug + Clone,
{
    ...
    fn get_alpha(&self) -> Result<f64> {
        if let Some(alpha) = self.alpha {
            ...
            Err(Error::InvalidValue("Must specify either alpha, span, or halflife".to_string()))
        }
    }
    ...
    T: Debug + Clone + Into<f64> + Copy,
{
    ...
    pub fn var(&self, ddof: usize) -> Result<Series<f64>> {
        ...
        let var_values: Vec<f64> = std_series.values().iter()
