    ...
    T: Debug + Clone,
{
    ...
    pub fn from_options(values: Vec<Option<T>>, name: Option<String>) -> Result<Self> {
        let na_values = values.into_iter().map(|opt| match opt {
            Some(v) => NA::Value(v),
            None => NA::NA,
        }).collect();
        Self::new(na_values, name)
        ...
        I: Debug + Clone + Eq + std::hash::Hash + std::fmt::Display,
    {
        ...
        Ok(NASeries { values, index: range_index, name })
    }
    ...
    pub fn fillna(&self, fill_value: T) -> Result<Self> {
        let filled_values: Vec<NA<T>> = self.values
            .iter()
            .map(|v| match v {
                NA::Value(_) => v.clone(),
