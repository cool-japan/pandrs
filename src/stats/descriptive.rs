...
use crate::stats::distributions::{Normal, TDistribution, Distribution};
...
pub fn describe(data: &[f64]) -> Result<StatisticalSummary> {
    if data.is_empty() {
        return Err(Error::InvalidValue("Cannot compute statistics for empty data".into()));
    }
    ...
    let coefficient_of_variation = if mean != 0.0 { std / mean.abs() } else { f64::NAN };
...
pub fn percentile(sorted_data: &[f64], p: f64) -> Result<f64> {
    if sorted_data.is_empty() {
        return Err(Error::InvalidValue("Cannot compute percentile for empty data".into()));
    }
    
    if p < 0.0 || p > 100.0 {
        return Err(Error::InvalidValue("Percentile must be between 0 and 100".into()));
    }
...
fn calculate_kurtosis(data: &[f64], mean: f64, std: f64) -> Result<f64> {
    ...
    let sample_kurtosis = ((n - 1.0) / ((n - 2.0) * (n - 3.0))) * 
                         ((n + 1.0) * excess_kurtosis + 6.0);
...
fn detect_outliers(data: &[f64], mean: f64, std: f64, q1: f64, q3: f64) -> Result<OutlierAnalysis> {
    ...
    let iqr_outliers: Vec<f64> = data.iter()
        .filter(|&&x| x < iqr_lower || x > iqr_upper)
        .copied()
        .collect();
    
    // Z-score method outliers (|z| > 3)
    let z_score_outliers: Vec<f64> = data.iter()
    ...
    for (i, var) in data.iter().enumerate() {
        if var.len() != n_obs {
            return Err(Error::DimensionMismatch(
                format!("Variable {} has different length than variable 0", i)
            ));
        }
    }
    
    if n_obs < 2 {
        return Err(Error::InvalidValue("At least 2 observations required".into()));
    }
...
pub fn pearson_correlation(x: &[f64], y: &[f64]) -> Result<f64> {
    if x.len() != y.len() {
        return Err(Error::DimensionMismatch("Variables must have same length".into()));
    }
...
fn calculate_ranks(data: &[f64]) -> Vec<f64> {
    let n = data.len();
    let mut indexed_data: Vec<(usize, f64)> = data.iter().enumerate()
        .map(|(i, &val)| (i, val))
        .collect();
...
pub fn covariance_matrix(data: &[Vec<f64>]) -> Result<Vec<Vec<f64>>> {
    ...
    for i in 0..n_vars {
        for j in 0..n_vars {
            let cov = data[i].iter().zip(data[j].iter())
...
pub fn describe_by_groups(
    values: &[f64],
    groups: &[String]
) -> Result<GroupedStatistics> {
    if values.len() != groups.len() {
        return Err(Error::DimensionMismatch("Values and groups must have same length".into()));
    }
    ...
    for (value, group) in values.iter().zip(groups.iter()) {
        grouped_data.entry(group.clone()).or_insert_with(Vec::new).push(*value);
    }
