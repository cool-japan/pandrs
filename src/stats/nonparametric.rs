...
use crate::stats::hypothesis::{TestResult, AlternativeHypothesis, EffectSize};
use crate::stats::distributions::{Normal, ChiSquared, Distribution};
...
pub fn kruskal_wallis_test(groups: &[&[f64]]) -> Result<TestResult> {
    ...
    if groups.len() < 2 {
        return Err(Error::InvalidValue("At least two groups are required".into()));
    }
    ...
    h_statistic = 12.0 / (n_total * (n_total + 1)) as f64 * h_statistic - 3.0 * (n_total + 1) as f64;
...
pub fn friedman_test(data: &[Vec<f64>]) -> Result<TestResult> {
    ...
    if k_conditions < 2 {
        return Err(Error::InvalidValue("At least two conditions required".into()));
    }
    ...
    for (i, subject) in data.iter().enumerate() {
        if subject.len() != k_conditions {
            return Err(Error::DimensionMismatch(
                format!("Subject {} has different number of conditions", i)
            ));
        }
    }
...
pub fn runs_test(sequence: &[bool]) -> Result<TestResult> {
    ...
    if sequence.len() < 2 {
        return Err(Error::InvalidValue("Sequence must have at least 2 elements".into()));
    }
    ...
    if n1 == 0 || n2 == 0 {
        return Err(Error::InvalidValue("Sequence must contain both true and false values".into()));
    }
    ...
    let variance_runs = (2.0 * n1_f * n2_f * (2.0 * n1_f * n2_f - n_total)) / 
    ...
    F: Fn(&[f64]) -> f64,
{
    ...
    if confidence_level <= 0.0 || confidence_level >= 1.0 {
        return Err(Error::InvalidValue("Confidence level must be between 0 and 1".into()));
    }
    ...
    F: Fn(&[f64], &[f64]) -> f64,
{
    ...
    let p_value = match alternative {
        AlternativeHypothesis::TwoSided => {
            let count = permutation_stats.iter()
