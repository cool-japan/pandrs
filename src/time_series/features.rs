...
impl TimeSeriesFeatureExtractor {
    ...
    fn extract_window_features(&self, values: &[f64]) -> Result<WindowFeatures> {
        ...
        for &window_size in &self.window_sizes {
            if window_size < values.len() {
                ...
                rolling_correlations.insert(window_size, self.rolling_correlation(values, window_size)?);
    ...
    fn extract_frequency_features(&self, values: &[f64]) -> Result<FrequencyFeatures> {
        ...
        let spectral_rolloff = frequencies.iter().zip(&psd)
            .find(|(_, power)| {
        ...
        let spectral_flux = psd.windows(2)
            .map(|window| (window[1] - window[0]).abs())
            .sum::<f64>() / psd.len() as f64;
    ...
    fn bollinger_bands(&self, values: &[f64], window: usize, std_dev: f64) -> Result<BollingerBandFeatures> {
        ...
        // Percentage within bands
        let within_bands = values.iter().zip(&upper_band).zip(&lower_band)
            .filter(|((value, upper), lower)| {
                value.is_finite() && upper.is_finite() && lower.is_finite() &&
    ...
    fn trend_strength(&self, values: &[f64], window: usize) -> Result<f64> {
        ...
        let sum_xy = x_values.iter().zip(window_values).map(|(x, y)| x * y).sum::<f64>();
        ...
        // Calculate R-squared
        let y_mean = sum_y / n;
        let ss_tot = window_values.iter().map(|y| (y - y_mean).powi(2)).sum::<f64>();
        let ss_res = x_values.iter().zip(window_values)
            .map(|(x, y)| {
    ...
    fn approximate_entropy(&self, values: &[f64], m: usize, r: f64) -> Result<f64> {
        ...
        for pattern_len in m..=m+1 {
            ...
            let phi_val = c.iter()
                .filter(|&&x| x > 0.0)
                .map(|&x| x.ln())
                .sum::<f64>() / (n - pattern_len + 1) as f64;
    ...
    fn permutation_entropy(&self, values: &[f64], order: usize) -> Result<f64> {
        ...
        for i in 0..total_patterns {
            ...
            let permutation = indices.into_iter()
                .enumerate()
                .map(|(rank, _)| rank)
                .collect::<Vec<_>>();
        ...
        let entropy = permutation_counts.values()
            .map(|&count| {
    ...
    fn fractal_dimension(&self, values: &[f64]) -> Result<f64> {
        ...
        let sum_xy = log_scales.iter().zip(&log_counts).map(|(x, y)| x * y).sum::<f64>();
    ...
    fn hurst_exponent(&self, values: &[f64]) -> Result<f64> {
        ...
        let mean = values.iter().sum::<f64>() / values.len() as f64;
        let cumulative_devs: Vec<f64> = values.iter()
            .scan(0.0, |acc, &x| {
                *acc += x - mean;
                Some(*acc)
            })
        ...
        for &n in &[10, 20, 50, 100] {
            if n < values.len() {
                let range = cumulative_devs[..n].iter().cloned().fold(f64::NEG_INFINITY, f64::max) -
                           cumulative_devs[..n].iter().cloned().fold(f64::INFINITY, f64::min);
                
                let std_dev = values[..n].iter()
                    .map(|&x| (x - mean).powi(2))
                    .sum::<f64>() / n as f64;
    ...
    fn detrended_fluctuation_analysis(&self, values: &[f64]) -> Result<f64> {
        ...
        let mean = values.iter().sum::<f64>() / values.len() as f64;
        let integrated: Vec<f64> = values.iter()
            .scan(0.0, |acc, &x| {
                *acc += x - mean;
                Some(*acc)
            })
        ...
        for &box_size in &[4, 8, 16, 32, 64] {
            if box_size < values.len() / 4 {
                ...
                for i in 0..num_boxes {
                    ...
                    let fluctuation = x_vals.iter().zip(box_data)
        ...
        let sum_xy = log_box_sizes.iter().zip(&log_fluctuations).map(|(x, y)| x * y).sum::<f64>();
...
mod tests {
    ...
    fn test_frequency_features() {
        ...
        assert_eq!(features.frequency.psd.len(), features.frequency.frequencies.len());
    ...
    fn test_complexity_features() {
        ...
        assert!(features.complexity.hurst_exponent >= 0.0 && features.complexity.hurst_exponent <= 1.0);
    }
