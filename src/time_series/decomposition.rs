...
impl SeasonalDecomposition {
    ...
    pub fn decompose(&self, ts: &TimeSeries) -> Result<DecompositionResult> {
        if ts.is_empty() {
            return Err(Error::InvalidInput("Cannot decompose empty time series".to_string()));
        }
    ...
    fn additive_decomposition(&self, ts: &TimeSeries, period: usize) -> Result<DecompositionResult> {
    ...
    fn multiplicative_decomposition(&self, ts: &TimeSeries, period: usize) -> Result<DecompositionResult> {
    ...
    fn extract_seasonal_additive(&self, detrended: &TimeSeries, period: usize) -> Result<TimeSeries> {
    ...
    fn extract_seasonal_multiplicative(&self, detrended: &TimeSeries, period: usize) -> Result<TimeSeries> {
    ...
    fn calculate_metrics(
        ...
        Ok(DecompositionMetrics {
            trend_variance_ratio: if original_var > 0.0 { trend_var / original_var } else { 0.0 },
            seasonal_variance_ratio: if original_var > 0.0 { seasonal_var / original_var } else { 0.0 },
            residual_variance_ratio: if original_var > 0.0 { residual_var / original_var } else { 0.0 },
            signal_to_noise_ratio: if noise_var > 0.0 { signal_var / noise_var } else { f64::INFINITY },
...
impl DecompositionResult {
    ...
    pub fn reconstruct(&self) -> Result<TimeSeries> {
        match self.method {
            ...
            DecompositionMethod::Multiplicative => {
                self.reconstruct_multiplicative()
            }
        }
    }
    ...
    pub fn quality_score(&self) -> f64 {
        ...
        let explained_variance = self.metrics.trend_variance_ratio + self.metrics.seasonal_variance_ratio;
...
mod tests {
    use super::*;
    use crate::time_series::core::{TimeSeries, TimeSeriesBuilder, Frequency};
    ...
    fn test_seasonal_decomposition() {
        ...
        let total_explained = result.metrics.trend_variance_ratio + result.metrics.seasonal_variance_ratio;
        assert!(total_explained > 0.7, "Decomposition should explain most variance");
    }
    ...
    fn test_multiplicative_decomposition() {
        ...
        let decomposer = SeasonalDecomposition::new(DecompositionMethod::Multiplicative)
            .with_period(7);
    ...
    fn test_decomposition_reconstruction() {
        ...
        for i in 0..ts.len() {
            ...
            assert!(diff < 1e-10, "Reconstruction should be very close to original");
        }
    }
    ...
    fn test_quality_metrics() {
        ...
        assert!(quality > 0.5, "Quality score should be reasonable for synthetic data");
