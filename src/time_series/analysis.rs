...
impl TrendAnalysis {
    ...
    pub fn analyze(ts: &TimeSeries) -> Result<TrendAnalysis> {
        ...
        if values.len() < 3 {
            return Err(Error::InvalidInput("Insufficient valid data points".to_string()));
        }
    ...
    fn linear_regression(values: &[f64]) -> Result<(f64, f64, f64)> {
        ...
        let ss_res = x_values.iter().zip(values)
            .map(|(x, y)| {
    ...
    fn slope_standard_error(values: &[f64], slope: f64, intercept: f64) -> Result<f64> {
        ...
        let rss = x_values.iter().zip(values)
            .map(|(x, y)| {
                let predicted = slope * x + intercept;
                (y - predicted).powi(2)
            })
...
impl SeasonalityAnalysis {
    ...
    pub fn analyze(ts: &TimeSeries, max_period: Option<usize>) -> Result<SeasonalityAnalysis> {
        ...
        for period in 2..=max_period {
            ...
            if strength > 0.05 { // Lower threshold for better detection
    ...
    fn detrend_series(values: &[f64]) -> Result<Vec<f64>> {
        ...
        // Remove trend
        let detrended: Vec<f64> = x_values.iter().zip(values.iter())
    ...
    fn find_fundamental_period(detected_periods: &HashMap<usize, f64>) -> Option<usize> {
        ...
        let mut periods: Vec<(usize, f64)> = detected_periods.iter()
...
impl StationarityTest {
    ...
    pub fn augmented_dickey_fuller(ts: &TimeSeries, lags: Option<usize>) -> Result<StationarityTest> {
        let values: Vec<f64> = (0..ts.len())
            .filter_map(|i| ts.values.get_f64(i))
            .filter(|v| v.is_finite())
            .collect();
...
impl AutocorrelationAnalysis {
    ...
    fn calculate_acf_confidence_intervals(values: &[f64], max_lags: usize) -> Result<Vec<(f64, f64)>> {
        let n = values.len() as f64;
...
impl ChangePointDetection {
    ...
    pub fn cusum_detection(ts: &TimeSeries, threshold: Option<f64>) -> Result<ChangePointDetection> {
        let values: Vec<f64> = (0..ts.len())
            .filter_map(|i| ts.values.get_f64(i))
            .filter(|v| v.is_finite())
            .collect();
    ...
    pub fn bayesian_detection(ts: &TimeSeries, prior_scale: Option<f64>) -> Result<super::analysis::ChangePointDetection> {
        let values: Vec<f64> = (0..ts.len())
            .filter_map(|i| ts.values.get_f64(i))
            .filter(|v| v.is_finite())
            .collect();
        ...
        for i in 2..(values.len() - 2) {
            ...
            if score > prior_scale * 10.0 { // Simplified threshold
                change_points.push(i);
