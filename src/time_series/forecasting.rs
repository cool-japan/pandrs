...
use crate::time_series::core::{TimeSeries, DateTimeIndex, Frequency, TimeSeriesData};
use crate::time_series::decomposition::{SeasonalDecomposition, DecompositionMethod};
...
impl Forecaster for SimpleMovingAverageForecaster {
    ...
    fn forecast(&self, periods: usize, confidence_level: f64) -> Result<ForecastResult> {
        let last_values = self.last_values.as_ref()
        ...
        let upper_ci_ts = TimeSeries::new(
            forecast_index,
            TimeSeriesData::from_vec(upper_values)
        )?;
    ...
    fn fit_metrics(&self, ts: &TimeSeries) -> Result<ForecastMetrics> {
        let fitted = self.fitted_values.as_ref()
            .ok_or_else(|| Error::InvalidOperation("Model not fitted".to_string()))?;
...
impl Forecaster for LinearTrendForecaster {
    fn fit(&mut self, ts: &TimeSeries) -> Result<()> {
        ...
        let y_values: Vec<f64> = (0..ts.len())
            .filter_map(|i| ts.values.get_f64(i))
            .collect();
        
        if y_values.len() != ts.len() {
            return Err(Error::InvalidInput("Missing values not supported".to_string()));
        }
        ...
        let fitted: Vec<f64> = x_values.iter()
            .map(|&x| slope * x + intercept)
            .collect();
        
        // Calculate residual standard deviation
        let residuals: Vec<f64> = y_values.iter().zip(&fitted)
    ...
    fn forecast(&self, periods: usize, confidence_level: f64) -> Result<ForecastResult> {
        let slope = self.slope.ok_or_else(|| Error::InvalidOperation("Model not fitted".to_string()))?;
        let intercept = self.intercept.ok_or_else(|| Error::InvalidOperation("Model not fitted".to_string()))?;
        let index = self.index.as_ref().ok_or_else(|| Error::InvalidOperation("Model not fitted".to_string()))?;
        let residual_std = self.residual_std.ok_or_else(|| Error::InvalidOperation("Model not fitted".to_string()))?;
...
impl Forecaster for ExponentialSmoothingForecaster {
    fn fit(&mut self, ts: &TimeSeries) -> Result<()> {
        ...
        match (&self.beta, &self.gamma, &self.seasonal_periods) {
            ...
            _ => return Err(Error::InvalidInput("Invalid smoothing configuration".to_string())),
        }
    ...
    fn forecast(&self, periods: usize, confidence_level: f64) -> Result<ForecastResult> {
        let level = self.level.ok_or_else(|| Error::InvalidOperation("Model not fitted".to_string()))?;
        let index = self.index.as_ref().ok_or_else(|| Error::InvalidOperation("Model not fitted".to_string()))?;
...
impl ExponentialSmoothingForecaster {
    fn fit_simple(&mut self, values: &[f64]) -> Result<()> {
        ...
        let residuals: Vec<f64> = values.iter().zip(&fitted)
            .map(|(actual, predicted)| actual - predicted)
            .collect();
        let residual_std = (residuals.iter().map(|r| r * r).sum::<f64>() / residuals.len() as f64).sqrt();
    ...
    fn fit_triple(&mut self, values: &[f64]) -> Result<()> {
        ...
        for i in 0..seasonal_periods {
            seasonal[i] = values[i] / (values.iter().take(seasonal_periods).sum::<f64>() / seasonal_periods as f64);
        }
        ...
        for (t, &value) in values.iter().enumerate() {
            ...
            if t < seasonal_periods {
                ...
                let prev_level = level;
                level = self.alpha * (value / seasonal[seasonal_idx]) + (1.0 - self.alpha) * (level + trend);
                trend = beta * (level - prev_level) + (1.0 - beta) * trend;
                seasonal[seasonal_idx] = gamma * (value / level) + (1.0 - gamma) * seasonal[seasonal_idx];
...
impl Forecaster for ArimaForecaster {
    ...
    fn forecast(&self, periods: usize, confidence_level: f64) -> Result<ForecastResult> {
        let ar_params = self.ar_params.as_ref()
            .ok_or_else(|| Error::InvalidOperation("Model not fitted".to_string()))?;
        let ma_params = self.ma_params.as_ref()
            .ok_or_else(|| Error::InvalidOperation("Model not fitted".to_string()))?;
        let index = self.index.as_ref()
            .ok_or_else(|| Error::InvalidOperation("Model not fitted".to_string()))?;
        let differenced = self.differenced_series.as_ref()
            .ok_or_else(|| Error::InvalidOperation("Model not fitted".to_string()))?;
        ...
        let mut last_values = differenced.iter().rev().take(self.p).cloned().collect::<Vec<_>>();
        last_values.reverse();
        let mut last_residuals = residuals.iter().rev().take(self.q).cloned().collect::<Vec<_>>();
        ...
        let forecast_ts = TimeSeries::new(
            forecast_index.clone(),
            TimeSeriesData::from_vec(forecasts)
        )?;
        let lower_ci_ts = TimeSeries::new(
            forecast_index.clone(),
            TimeSeriesData::from_vec(lower_values)
...
fn calculate_forecast_metrics(ts: &TimeSeries, fitted: &[f64]) -> Result<ForecastMetrics> {
    let actual_values: Vec<f64> = (0..ts.len())
        .filter_map(|i| ts.values.get_f64(i))
        .collect();
    ...
    let valid_pairs: Vec<(f64, f64)> = actual_values.iter()
        .zip(fitted.iter())
    ...
    // Mean Absolute Error
    let mae = valid_pairs.iter()
        .map(|(actual, fitted)| (actual - fitted).abs())
        .sum::<f64>() / n;
    
    // Mean Squared Error
    let mse = valid_pairs.iter()
...
mod tests {
    ...
    fn test_linear_trend_forecaster() {
        ...
        assert!(last_forecast > first_forecast, "Should show increasing trend");
    }
