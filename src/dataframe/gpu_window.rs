...
use crate::dataframe::jit_window::{
    JitWindowContext, JitWindowStats, WindowOpType, WindowFunctionKey, JitDataFrameWindowExt
};
use crate::dataframe::enhanced_window::{
    DataFrameWindowExt, DataFrameRolling, DataFrameExpanding, DataFrameEWM
};
use crate::gpu::{GpuManager, GpuConfig, GpuError, get_gpu_manager};
...
impl GpuWindowStats {
    ...
    pub fn record_gpu_execution(&mut self, kernel_time_ns: u64, speedup_ratio: f64) {
        ...
        if self.gpu_executions == 1 {
            ...
            self.average_gpu_speedup = (self.average_gpu_speedup * (self.gpu_executions - 1) as f64 + speedup_ratio) / self.gpu_executions as f64;
        }
    }
...
impl GpuWindowContext {
    ...
    pub fn with_config(jit_enabled: bool, jit_threshold: u64, gpu_enabled: bool, gpu_threshold_size: usize) -> Result<Self> {
    ...
    pub fn should_use_gpu(&self, data_size: usize, op_type: &WindowOpType) -> bool {
        ...
        match op_type {
            ...
            WindowOpType::RollingMean | WindowOpType::RollingSum | 
            WindowOpType::ExpandingMean | WindowOpType::ExpandingSum => {
                data_size >= self.gpu_threshold_size
            },
            // These operations benefit significantly from GPU parallelism
            WindowOpType::RollingStd | WindowOpType::RollingVar |
    ...
    pub fn execute_gpu_operation(&self, 
                                 key: &WindowFunctionKey, 
                                 data: &[f64],
                                 window_size: usize) -> Result<Vec<f64>> {
        ...
        let total_memory_required = data_bytes + result_bytes + (window_size * std::mem::size_of::<f64>());
        
        let device_status = self.gpu_manager.device_info();
        if let Some(free_memory) = device_status.free_memory {
            if total_memory_required > free_memory / 2 { // Use only half of available memory
                let mut stats = self.gpu_stats.lock().unwrap();
                stats.record_cpu_fallback();
                return Err(Error::InvalidOperation("Insufficient GPU memory for operation".to_string()));
            }
        }
        ...
        let result = match &key.operation {
            WindowOpType::RollingMean => {
                self.gpu_rolling_mean(data, window_size)
            },
            WindowOpType::RollingSum => {
                self.gpu_rolling_sum(data, window_size)
            },
            WindowOpType::RollingStd => {
                self.gpu_rolling_std(data, window_size)
            },
            WindowOpType::RollingVar => {
                self.gpu_rolling_var(data, window_size)
            },
            WindowOpType::ExpandingMean => {
                self.gpu_expanding_mean(data)
            },
            WindowOpType::ExpandingSum => {
                self.gpu_expanding_sum(data)
            },
            ...
            _ => {
                ...
                let mut stats = self.gpu_stats.lock().unwrap();
                stats.record_cpu_fallback();
                return Err(Error::InvalidOperation(format!("GPU kernel not implemented for {:?}", key.operation)));
            }
        };
    ...
    fn gpu_rolling_mean(&self, data: &[f64], window_size: usize) -> Result<Vec<f64>> {
        #[cfg(feature = "cuda")]
        {
            ...
            for i in window_size - 1..data.len() {
                let window_start = if i >= window_size - 1 { i - window_size + 1 } else { 0 };
                let window_end = i + 1;
            ...
            let data_bytes = data.len() * std::mem::size_of::<f64>();
            let mut stats = self.gpu_stats.lock().unwrap();
            stats.record_data_transfer(transfer_start.elapsed().as_nanos() as u64, data_bytes as u64 * 2); // Input + output
    ...
    fn gpu_rolling_std(&self, data: &[f64], window_size: usize) -> Result<Vec<f64>> {
        ...
        Ok(variance.into_iter().map(|v| if v.is_nan() { f64::NAN } else { v.sqrt() }).collect())
    }
...
impl<'a> GpuDataFrameRolling<'a> {
    ...
    pub fn new(dataframe: &'a DataFrame, window_size: usize, gpu_context: &'a GpuWindowContext) -> Self {
        Self {
            dataframe,
            window_size,
            gpu_context,
    ...
    fn execute_rolling_operation(self, op_type: WindowOpType) -> Result<DataFrame> {
        ...
        let target_columns = if let Some(cols) = self.columns {
            ...
            // Get numeric columns
            self.dataframe.column_names().into_iter()
                .filter(|col_name| {
        ...
        for col_name in target_columns {
            if let Ok(series) = self.dataframe.get_column::<f64>(&col_name) {
                ...
                let processed_data = if self.gpu_context.should_use_gpu(data_size, &op_type) {
                    ...
                    match self.gpu_context.execute_gpu_operation(&key, data, self.window_size) {
    ...
    fn fallback_to_jit_operation(&self, op_type: &WindowOpType, data: &[f64]) -> Result<Vec<f64>> {
        ...
        match op_type {
            ...
            _ => Err(Error::InvalidOperation(format!("Fallback not implemented for {:?}", op_type)))
        }
    }
    ...
    fn cpu_rolling_mean(&self, data: &[f64]) -> Result<Vec<f64>> {
        ...
        for i in self.window_size - 1..data.len() {
            let window_start = if i >= self.window_size - 1 { i - self.window_size + 1 } else { 0 };
            let window_end = i + 1;
            let window_data = &data[window_start..window_end];
            result[i] = window_data.iter().sum::<f64>() / window_data.len() as f64;
        }
...
pub trait GpuDataFrameWindowExt {
    ...
    fn gpu_rolling<'a>(&'a self, window_size: usize, gpu_context: &'a GpuWindowContext) -> GpuDataFrameRolling<'a>;
}
