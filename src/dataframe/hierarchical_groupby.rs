...
use crate::dataframe::groupby::{AggFunc, NamedAgg, CustomAggFn};
use crate::dataframe::multi_index_results::{MultiIndexDataFrame, MultiIndexColumn, ToMultiIndex};
...
impl HierarchicalKey {
    ...
    pub fn partial_key(&self, up_to_level: usize) -> Self {
        ...
        let level_names = self.level_names.iter().take(up_to_level + 1).cloned().collect();
...
impl GroupHierarchy {
    ...
    fn add_row_to_hierarchy(&mut self, row_idx: usize, row_path: &[String]) -> Result<()> {
        for (level, value) in row_path.iter().enumerate() {
            ...
            let hierarchical_key = HierarchicalKey::new(
                current_path.clone(),
                self.level_columns[..=level].to_vec(),
            );
            
            // Update level index
            self.level_index.entry(level).or_default().push(hierarchical_key.clone());
    ...
    pub fn get_statistics(&self) -> HierarchyStatistics {
        ...
        if self.total_groups > 0 {
            let total_size: usize = self.get_all_leaf_nodes().iter()
...
impl HierarchicalDataFrameGroupBy {
    ...
    pub fn get_group_keys(&self) -> Vec<HierarchicalKey> {
        match (&self.current_context.current_level, &self.current_context.current_path) {
            (Some(level), None) => {
                // All groups at a specific level
                self.hierarchy.get_level_nodes(*level)
                    .into_iter()
            ...
            (None, None) => {
                ...
                self.hierarchy.get_all_leaf_nodes()
                    .into_iter()
                    .map(|node| node.full_key.clone())
                    .collect()
            }
    ...
    fn navigate_to_group_by_path(&self, path: &[String]) -> Option<&GroupNode> {
        let key = HierarchicalKey::new(path.to_vec(), self.hierarchy.level_columns[..path.len()].to_vec());
    ...
    pub fn agg_hierarchical(&self, hierarchical_aggs: Vec<HierarchicalAgg>) -> Result<DataFrame> {
        ...
        for level in 0..max_levels {
            let level_name = self.hierarchy.level_columns.get(level)
                .map(|s| s.clone())
                .unwrap_or_else(|| format!("level_{}", level));
            
            let level_values: Vec<String> = group_keys.iter()
    ...
    fn calculate_hierarchical_aggregation(
        ...
        match func {
            ...
            _ => Err(Error::NotImplemented(format!("Aggregation function {:?} not yet implemented for hierarchical groupby", func)))
        }
    }
    ...
    pub fn size(&self) -> Result<DataFrame> {
        ...
        // Add size column
        let sizes: Vec<String> = group_keys.iter()
            .map(|key| {
                if let Some(node) = self.hierarchy.navigate_to_group(key) {
                    if self.current_context.include_children {
    ...
    pub fn agg_hierarchical_multi_index(&self, hierarchical_aggs: Vec<HierarchicalAgg>) -> Result<MultiIndexDataFrame> {
    ...
    pub fn cross_level_agg(&self, column: &str, func: AggFunc, source_level: usize, target_level: usize) -> Result<DataFrame> {
        if source_level >= self.hierarchy.max_depth || target_level >= self.hierarchy.max_depth {
            return Err(Error::InvalidValue("Level exceeds hierarchy depth".to_string()));
        }
        ...
        for level in 0..=target_level {
            ...
            let level_values: Vec<String> = target_groups.iter()
    ...
    pub fn nested_transform<F>(&self, column: &str, transform_fn: F, level: usize) -> Result<DataFrame>
    where
    ...
    pub fn inter_level_ratio(&self, column: &str, parent_level: usize, child_level: usize) -> Result<DataFrame> {
        if parent_level >= child_level || child_level >= self.hierarchy.max_depth {
            return Err(Error::InvalidValue("Invalid level relationship".to_string()));
        }
        ...
        for level in 0..=child_level {
            ...
            let level_values: Vec<String> = child_nodes.iter()
                .map(|node| node.full_key.get_level(level).unwrap_or("").to_string())
                .collect();
        ...
        for child_node in &child_nodes {
            ...
            let child_sum = self.calculate_hierarchical_aggregation(
                column, AggFunc::Sum, child_node, &None, false
            )?;
            
            let parent_sum = if let Some(parent_node) = self.hierarchy.navigate_to_group(&parent_key) {
                self.calculate_hierarchical_aggregation(
                    column, AggFunc::Sum, parent_node, &None, true
                )?
            } else {
                f64::NAN
            };
    ...
    pub fn hierarchical_filter<F>(&self, column: &str, level: usize, filter_fn: F) -> Result<HierarchicalDataFrameGroupBy>
    where
        F: Fn(f64) -> bool + Send + Sync,
    {
        ...
        for node in &level_nodes {
            let agg_value = self.calculate_hierarchical_aggregation(
                column, AggFunc::Sum, node, &None, true
            )?;
    ...
    pub fn cross_level_comparison(&self, column: &str, base_level: usize, compare_levels: Vec<usize>) -> Result<DataFrame> {
        let mut result = DataFrame::new();
        ...
        for level in 0..=base_level {
            ...
            let level_values: Vec<String> = base_nodes.iter()
                .map(|node| node.full_key.get_level(level).unwrap_or("").to_string())
                .collect();
        ...
        for base_node in &base_nodes {
            let base_value = self.calculate_hierarchical_aggregation(
                column, AggFunc::Sum, base_node, &None, true
            )?;
        ...
        for &compare_level in &compare_levels {
            ...
            for base_node in &base_nodes {
                let compare_value = if compare_level > base_level {
                    ...
                    let child_sum: f64 = child_nodes.iter()
                        .map(|child| {
                            self.calculate_hierarchical_aggregation(
                                column, AggFunc::Sum, child, &None, false
                            ).unwrap_or(0.0)
                        })
                        .sum();
                    child_sum
    ...
    pub fn nested_rollup(&self, column: &str, func: AggFunc) -> Result<DataFrame> {
        ...
        all_groups.sort_by(|a, b| {
            a.0.cmp(&b.0).then_with(|| a.1.full_key.cmp(&b.1.full_key))
        });
        ...
        for level in 0..max_levels {
            ...
            let level_values: Vec<String> = all_groups.iter()
                .map(|(_, node)| {
                    node.full_key.get_level(level).unwrap_or("").to_string()
                })
                .collect();
        ...
        let rollup_levels: Vec<String> = all_groups.iter()
...
mod tests {
    ...
    fn test_hierarchical_key() {
        let key = HierarchicalKey::new(
            ...
            vec!["category".to_string(), "region".to_string(), "product".to_string()],
        );
    ...
    fn test_hierarchical_agg_builder() {
        ...
        assert_eq!(agg.level_functions[0], (0, AggFunc::Sum, "total_sales".to_string()));
        assert_eq!(agg.level_functions[1], (1, AggFunc::Mean, "avg_sales".to_string()));
    }
    ...
    fn test_cross_level_aggregation() {
        ...
        df.add_column("product".to_string(), Series::new(
        ...
        let result = hierarchical_gb.cross_level_agg("sales", AggFunc::Sum, 2, 1).unwrap();
        assert!(result.row_count() > 0);
    ...
    fn test_nested_rollup() {
        ...
        let result = hierarchical_gb.nested_rollup("sales", AggFunc::Sum).unwrap();
        assert!(result.row_count() > 0);
    ...
    fn test_utility_functions() {
        ...
        assert!(comprehensive_aggs.iter().any(|agg| agg.level_functions[0].1 == AggFunc::Sum));
        assert!(comprehensive_aggs.iter().any(|agg| agg.level_functions[0].1 == AggFunc::Mean));
        assert!(comprehensive_aggs.iter().any(|agg| agg.level_functions[0].1 == AggFunc::Count));
    ...
    fn test_error_handling() {
        ...
        df.add_column("valid_column".to_string(), Series::new(
