...
impl<'a> StatPlotBuilder<'a> {
    ...
    pub fn correlation_matrix<P: AsRef<Path>>(self, path: P) -> Result<()> {
        ...
        if numeric_columns.len() < 2 {
            return Err(Error::InvalidValue("At least 2 numeric columns required for correlation matrix".to_string()));
        }
    ...
    pub fn pair_plot<P: AsRef<Path>>(self, columns: Option<&[String]>, path: P) -> Result<()> {
        ...
        if cols.len() < 2 {
            return Err(Error::InvalidValue("At least 2 columns required for pair plot".to_string()));
        }
    ...
    pub fn distribution_plots<P: AsRef<Path>>(self, path_prefix: P) -> Result<Vec<String>> {
        ...
        for column in &numeric_columns {
            ...
            let path = format!("{}_{}_dist.png", path_prefix.as_ref().to_string_lossy(), column);
    ...
    pub fn dashboard<P: AsRef<Path>>(self, path: P) -> Result<()> {
        ...
        if numeric_columns.is_empty() {
            return Err(Error::InvalidValue("No numeric columns found for dashboard".to_string()));
        }
    ...
    fn get_numeric_columns(&self) -> Result<Vec<String>> {
        ...
        for col_name in self.df.column_names() {
            ...
            let is_numeric = values.iter().all(|v| {
                v.trim().parse::<f64>().is_ok() || v.trim().is_empty()
            });
        ...
        for (i, col1) in columns.iter().enumerate() {
            ...
            for (j, col2) in columns.iter().enumerate() {
                ...
                if values1.len() != values2.len() {
                    return Err(Error::InvalidValue("Column lengths don't match".to_string()));
                }
    ...
    fn get_numeric_values(&self, column: &str) -> Result<Vec<f64>> {
        ...
        for value in string_values {
            if let Ok(num) = value.parse::<f64>() {
                ...
                return Err(Error::InvalidValue(format!("Non-numeric value '{}' in column '{}'", value, column)));
            }
        }
    ...
    fn plot_heatmap<P: AsRef<Path>>(
        ...
        output.push_str(&format!("Correlation Matrix Heatmap: {}\n\n", self.config.title));
    ...
    fn plot_column_histogram<P: AsRef<Path>>(
        ...
        if values.is_empty() {
            return Err(Error::InvalidValue(format!("No numeric values in column '{}'", column)));
        }
    ...
    fn create_grouped_box_plots<P: AsRef<Path>>(
        ...
        for value_col in value_columns {
            ...
            for group in &unique_groups {
                ...
                let group_data: Vec<f64> = group_column_values.iter()
                ...
                if !group_data.is_empty() {
                    ...
                    output.push_str(&format!("Mean: {:.2}, Std: {:.2}, Count: {}\n", mean, std_dev, group_data.len()));
                } else {
    ...
    fn create_dashboard<P: AsRef<Path>>(&self, numeric_columns: &[String], path: P) -> Result<()> {
        ...
        for column in numeric_columns {
            ...
            if !values.is_empty() {
                ...
                output.push_str(&format!("  {}:\n", column));
                output.push_str(&format!("    Count: {}, Mean: {:.2}, Std: {:.2}\n", values.len(), mean, std_dev));
...
impl EnhancedPlotExt for DataFrame {
    ...
    fn plot_scatter<P: AsRef<Path>>(&self, x_col: &str, y_col: &str, path: P) -> Result<()> {
        ...
        let x_numeric: Result<Vec<f64>> = x_values.iter()
            .map(|v| v.parse::<f64>().map_err(|e| Error::InvalidValue(e.to_string())))
            .collect();
        let y_numeric: Result<Vec<f64>> = y_values.iter()
            .map(|v| v.parse::<f64>().map_err(|e| Error::InvalidValue(e.to_string())))
            .collect();
...
impl InteractivePlot {
    ...
    pub fn to_html<P: AsRef<Path>>(&self, path: P) -> Result<()> {
        ...
        html.push_str(".plot-container { margin: 20px 0; padding: 20px; border: 1px solid #ddd; }\n");
        html.push_str("</style>\n");
        html.push_str("</head>\n<body>\n");
        html.push_str(&format!("<h1>{}</h1>\n", self.config.title));
        html.push_str("<div class='plot-container'>\n");
        html.push_str("<h2>Interactive Data Visualization</h2>\n");
        html.push_str(&format!("<p>Dataset: {} rows, {} columns</p>\n", 
                              self.data.row_count(), self.data.column_names().len()));
