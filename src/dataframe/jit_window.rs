...
use crate::dataframe::enhanced_window::{
    ...
    DataFrameRollingOps, DataFrameExpandingOps, DataFrameEWMOps
};
...
impl JitWindowStats {
    ...
    pub fn average_speedup_ratio(&self) -> f64 {
        if self.jit_executions > 0 {
            (self.time_saved_ns as f64 / self.jit_executions as f64) / 1_000_000.0 // Convert to ms
        } else {
            1.0
        }
    }
    ...
    fn compile_window_function(&self, key: &WindowFunctionKey) -> Result<JitFunction> {
        ...
        let function = match &key.operation {
            WindowOpType::RollingMean => {
                JitFunction::new("rolling_mean", |window: Vec<f64>| {
                    if window.is_empty() { return f64::NAN; }
                    window.iter().sum::<f64>() / window.len() as f64
                })
            },
            WindowOpType::RollingSum => {
                JitFunction::new("rolling_sum", |window: Vec<f64>| {
                    window.iter().sum::<f64>()
                })
            },
            WindowOpType::RollingMin => {
                JitFunction::new("rolling_min", |window: Vec<f64>| {
                    window.iter().fold(f64::INFINITY, |a, &b| a.min(b))
                })
            },
            WindowOpType::RollingMax => {
                JitFunction::new("rolling_max", |window: Vec<f64>| {
                    window.iter().fold(f64::NEG_INFINITY, |a, &b| a.max(b))
                })
            },
            WindowOpType::RollingStd => {
                JitFunction::new("rolling_std", |window: Vec<f64>| {
                    if window.len() <= 1 { return f64::NAN; }
            ...
            WindowOpType::RollingVar => {
                JitFunction::new("rolling_var", |window: Vec<f64>| {
                    if window.len() <= 1 { return f64::NAN; }
                    let mean = window.iter().sum::<f64>() / window.len() as f64;
                    window.iter()
                        .map(|x| (x - mean).powi(2))
            ...
            WindowOpType::RollingCount => {
                JitFunction::new("rolling_count", |window: Vec<f64>| {
                    window.len() as f64
                })
            },
            ...
            WindowOpType::ExpandingSum => {
                JitFunction::new("expanding_sum", |window: Vec<f64>| {
                    window.iter().sum::<f64>()
                })
            },
            ...
            _ => {
                return Err(Error::InvalidOperation(format!("JIT compilation not yet implemented for operation: {:?}", key.operation)));
            }
        };
        ...
        // Record compilation statistics
        {
            ...
            match &key.operation {
                WindowOpType::RollingMean | WindowOpType::RollingSum | WindowOpType::RollingMin | 
                WindowOpType::RollingMax | WindowOpType::RollingStd | WindowOpType::RollingVar | 
                WindowOpType::RollingCount | WindowOpType::RollingMedian | WindowOpType::RollingQuantile(_) => {
                    stats.record_rolling_compilation(compilation_time);
                },
                WindowOpType::ExpandingMean | WindowOpType::ExpandingSum | WindowOpType::ExpandingStd | 
                WindowOpType::ExpandingVar | WindowOpType::ExpandingMin | WindowOpType::ExpandingMax | 
                WindowOpType::ExpandingCount | WindowOpType::ExpandingMedian => {
        ...
        F: FnOnce(&DataFrameRollingOps<'a>) -> Result<DataFrame>,
    {
        ...
        match self.jit_context.get_or_compile_function(&key) {
            ...
            Err(e) => {
                ...
                println!("JIT compilation failed, falling back to standard implementation: {}", e);
                fallback(&self.inner)
            }
        }
    }
