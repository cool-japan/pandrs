...
use crate::dataframe::hierarchical_groupby::{HierarchicalKey, HierarchicalAgg};
...
impl MultiIndexDataFrame {
    ...
    pub fn select_columns_by_level(&self, level: usize, values: &[String]) -> Result<MultiIndexDataFrame> {
        let selected_columns: Vec<_> = self.column_index.iter()
            .enumerate()
            .filter(|(_, col)| {
                col.levels.get(level).map_or(false, |v| values.contains(v))
            })
            .collect();
        
        if selected_columns.is_empty() {
            return Err(Error::InvalidValue("No columns match the selection criteria".to_string()));
        }
    ...
    pub fn group_columns_by_parent(&self, depth: usize) -> BTreeMap<Vec<String>, Vec<&MultiIndexColumn>> {
    ...
    pub fn column_summary(&self) -> ColumnHierarchySummary {
        ...
        for column in &self.column_index {
            ...
            let level_summary = summary.levels_summary.entry(depth).or_insert_with(|| LevelSummary {
                count: 0,
            ...
            if column.depth() > 1 {
                level_summary.unique_parents.insert(column.parent_levels().join("."));
            }
    ...
    pub fn pivot_columns(&self, new_level_order: &[usize]) -> Result<MultiIndexDataFrame> {
        if new_level_order.len() != self.metadata.max_column_depth {
            return Err(Error::InvalidValue("New level order must match column depth".to_string()));
        }
    ...
    pub fn to_hierarchical_string(&self, max_rows: Option<usize>) -> String {
        ...
        // Header information
        output.push_str(&format!("MultiIndex DataFrame ({}x{})\n", 
                                self.data.row_count(), 
                                self.column_index.len()));
        output.push_str(&format!("Index levels: {}\n", self.index_hierarchy.join(" | ")));
        output.push_str(&format!("Column hierarchy depth: {}\n", self.metadata.max_column_depth));
        output.push_str("\n");
...
impl MultiIndexDataFrameBuilder {
    ...
    pub fn add_hierarchical_aggregation(&mut self,
                                       base_column: &str,
                                       agg_func: &str,
                                       level_results: Vec<(usize, Series<String>)>) -> Result<()> {
        for (level, data) in level_results {
            let column_spec = MultiIndexColumn::new(
                vec![base_column.to_string(), format!("level_{}", level), agg_func.to_string()],
                vec!["column".to_string(), "level".to_string(), "function".to_string()],
                "f64".to_string(),
            );
        ...
        self.aggregation_metadata.push(format!("{}_{}", base_column, agg_func));
        Ok(())
    }
    ...
    pub fn build(self) -> Result<MultiIndexDataFrame> {
        ...
        for index_col in &self.index_columns {
            ...
            let index_series: Series<String> = Series::new(placeholder_data, Some(index_col.clone()))?;
    ...
    fn to_multi_index(&self, agg_specs: &[HierarchicalAgg]) -> Result<MultiIndexDataFrame> {
        ...
        for col_name in &column_names {
            if !agg_specs.iter().any(|spec| {
                spec.level_functions.iter().any(|(_, _, alias)| alias == col_name)
            }) {
...
pub mod utils {
    ...
    pub fn create_aggregation_hierarchy(
        ...
        for &column in columns {
            for &function in functions {
                result.push(simple_multi_index_column(
                    vec![column, function],
                    "f64",
                ));
            }
    ...
    pub fn merge_multi_index_dataframes(
        ...
        if dataframes.is_empty() {
            return Err(Error::InvalidValue("Cannot merge empty list of DataFrames".to_string()));
        }
...
mod tests {
    ...
    fn test_multi_index_column() {
        let column = MultiIndexColumn::new(
            ...
            vec!["metric".to_string(), "period".to_string(), "function".to_string()],
            "f64".to_string(),
        );
