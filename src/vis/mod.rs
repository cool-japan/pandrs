...
impl DataFrame {
    ...
    pub fn plot_xy<P: AsRef<Path>>(&self, x_col: &str, y_col: &str, path: P, config: PlotConfig) -> Result<()> {
        ...
        if !self.contains_column(x_col) {
            return Err(PandRSError::Column(format!("列 '{}' が存在しません", x_col)));
        }
        if !self.contains_column(y_col) {
            return Err(PandRSError::Column(format!("列 '{}' が存在しません", y_col)));
        }
...
fn plot_xy<P: AsRef<Path>>(x: &[f32], y: &[f32], path: P, config: PlotConfig) -> Result<()> {
    if x.len() != y.len() {
        return Err(PandRSError::Consistency("XとYの長さが一致しません".to_string()));
    }
    ...
    chart_string.push_str(&format!("X軸: {}, Y軸: {}\n\n", config.x_label, config.y_label));
    
    // プロット描画
    let chart_result = match config.plot_type {
        PlotType::Line => {
            Chart::new(config.width as u32, config.height as u32, x[0], x[x.len()-1])
    ...
    match config.format {
        ...
        OutputFormat::TextFile => {
            ...
            file.write_all(chart_string.as_bytes()).map_err(PandRSError::Io)?;
            Ok(())
        }
    }
}