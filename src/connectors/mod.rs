...
pub use database::{
    DatabaseConnector, DatabaseConfig, DatabaseConnectorFactory,
    WriteMode, TableInfo, ColumnInfo,
    SQLiteConnector,
};
...
pub use cloud::{
    CloudConnector, CloudConfig, CloudProvider, CloudCredentials,
    CloudConnectorFactory, FileFormat, CloudObject, ObjectMetadata,
...
impl DataConnector {
    ...
    pub fn from_connection_string(connection_string: &str) -> Result<Self> {
        if connection_string.starts_with("s3://") {
            ...
            Err(Error::InvalidOperation(format!("Unsupported connection string: {}", connection_string)))
        }
    }
}
...
impl DataSource {
    ...
    pub async fn read_sql(&self, query: &str) -> Result<DataFrame> {
        ...
        match &self.connector {
            ...
            #[cfg(not(feature = "sql"))]
            DataConnector::SQLite(_) => Err(Error::FeatureNotAvailable("SQL feature not enabled".to_string())),
    ...
    pub async fn read_cloud(&self, bucket: &str, key: &str) -> Result<DataFrame> {
        ...
        let format = FileFormat::from_extension(key)
            .unwrap_or(FileFormat::CSV { delimiter: ',', has_header: true });
    ...
    pub async fn write_sql(&self, df: &DataFrame, table_name: &str) -> Result<()> {
        ...
        match &self.connector {
            ...
            #[cfg(feature = "sql")]
            DataConnector::PostgreSQL(db) => db.write_table(df, table_name, WriteMode::Replace).await,
            #[cfg(not(feature = "sql"))]
            DataConnector::SQLite(_) => Err(Error::FeatureNotAvailable("SQL feature not enabled".to_string())),
            _ => Err(Error::InvalidOperation(
        ...
        if connection_string.starts_with("s3://") || 
           ...
           connection_string.starts_with("azure://") {
            ...
            if parts.len() >= 2 {
                ...
                Err(Error::InvalidOperation("Invalid cloud storage path".to_string()))
            }
        } else {
            // Database query
...
mod tests {
    ...
    fn test_data_connector_from_connection_string() {
        ...
        assert!(matches!(sqlite_connector.unwrap(), DataConnector::SQLite(_)));
    }
