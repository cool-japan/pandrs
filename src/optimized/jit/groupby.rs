...
use crate::optimized::split_dataframe::group::{GroupBy, CustomAggregation, AggregateOp};
...
use super::parallel::{parallel_sum_f64, parallel_mean_f64_value, parallel_std_f64_value, parallel_min_f64, parallel_max_f64};
use std::sync::Arc;
...
pub trait GroupByJitExt<'a> {
    ...
    fn parallel_sum_jit(&self, column: &str, result_name: &str, config: Option<ParallelConfig>) -> Result<OptimizedDataFrame>;
    
    /// Apply parallel JIT-accelerated mean operation
    fn parallel_mean_jit(&self, column: &str, result_name: &str, config: Option<ParallelConfig>) -> Result<OptimizedDataFrame>;
    
    /// Apply parallel JIT-accelerated standard deviation operation
    fn parallel_std_jit(&self, column: &str, result_name: &str, config: Option<ParallelConfig>) -> Result<OptimizedDataFrame>;
    
    /// Apply custom JIT-compiled aggregation function
    fn aggregate_jit<F>(&self, column: &str, func: JitFunction<F>, result_name: &str) -> Result<OptimizedDataFrame>
    where
...
impl<'a> GroupByJitExt<'a> for GroupBy<'a> {
    ...
    fn parallel_mean_jit(&self, column: &str, result_name: &str, config: Option<ParallelConfig>) -> Result<OptimizedDataFrame> {
        let custom_fn = Arc::new(move |values: &[f64]| {
            parallel_mean_f64_value(values, config.clone())
        });
    ...
    fn parallel_std_jit(&self, column: &str, result_name: &str, config: Option<ParallelConfig>) -> Result<OptimizedDataFrame> {
        let custom_fn = Arc::new(move |values: &[f64]| {
            parallel_std_f64_value(values, config.clone())
        });
...
pub mod aggregations {
    ...
    pub fn harmonic_mean() -> JitAggregation<impl Fn(&[f64]) -> f64 + Send + Sync + Clone> {
        JitAggregation::new("harmonic_mean", |values: &[f64]| -> f64 {
            ...
            if count == 0 { 0.0 } else { count as f64 / reciprocal_sum }
        })
    }
