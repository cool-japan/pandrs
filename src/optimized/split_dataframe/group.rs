...
impl OptimizedDataFrame {
    ...
    /// # Returns
    /// * `Result<GroupBy>` - Grouping results
    pub fn group_by_with_options<I, S>(&self, columns: I, as_multi_index: bool) -> Result<GroupBy<'_>>
    where
        I: IntoIterator<Item = S>,
        S: AsRef<str>,
    {
...
impl<'a> GroupBy<'a> {
        ...
        I: IntoIterator<Item = (String, AggregateOp, String)>,
    {
        ...
        let group_tuples: Mutex<Vec<Vec<String>>> = Mutex::new(Vec::with_capacity(self.groups.len()));
        ...
        (0..group_keys.len()).into_par_iter().for_each(|group_idx| {
            ...
            // Add values for grouping keys
            {
                ...
                for (i, col_name) in self.group_by_columns.iter().enumerate() {
                    group_key_map.get_mut(col_name).unwrap().push(key[i].clone());
                }
        ...
        if self.create_multi_index && self.group_by_columns.len() > 1 {
            ...
            let names = Some(self.group_by_columns.iter()
        ...
        I: IntoIterator<Item = CustomAggregation>,
    {
        ...
        let agg_result_data: Mutex<HashMap<String, Vec<f64>>> = Mutex::new(
            aggregations.iter()
                .map(|agg| (agg.result_name.clone(), Vec::with_capacity(self.groups.len())))
                .collect()
        );
    ...
    /// # Returns
    /// * `Result<OptimizedDataFrame>` - DataFrame containing aggregation results
    pub fn par_custom<F>(&self, column: &str, result_name: &str, func: F) -> Result<OptimizedDataFrame>
    where
        ...
        F: Fn(&OptimizedDataFrame) -> bool + Send + Sync + 'static,
    {
        ...
        group_keys.into_par_iter().zip(row_indices_list.into_par_iter())
        ...
        I: IntoIterator<Item = CustomAggregation>,
    {
        ...
        for (key, row_indices) in &self.groups {
            ...
            for agg in &aggregations {
                ...
                agg_result_data.get_mut(&agg.result_name).unwrap().push(result_value);
            }
        }
    ...
    fn calculate_aggregation(&self, col: &Column, op: AggregateOp, row_indices: &[usize]) -> Result<f64> {
        match (col, op) {
            ...
            (_, AggregateOp::Count) => {
                Ok(row_indices.len() as f64)
            },
            (_, AggregateOp::Custom) => {
                Err(Error::OperationFailed(
                    "Custom aggregation requires a custom function, use aggregate_custom instead".to_string()
                ))
            },
            _ => {
        ...
        I: IntoIterator<Item = (String, AggregateOp, String)>,
    {
        ...
        if self.create_multi_index && self.group_by_columns.len() > 1 {
            ...
            let mut tuples: Vec<Vec<String>> = Vec::with_capacity(group_key_data.values().next().unwrap().len());
