...
use crate::column::{Column, ColumnType, ColumnTrait};
use crate::error::{Error, Result};
use crate::optimized::dataframe::OptimizedDataFrame;
use crate::optimized::jit::simd::{simd_sum_f64, simd_mean_f64, simd_max_f64, simd_min_f64, simd_sum_i64, simd_mean_i64, simd_max_i64, simd_min_i64};
...
impl OptimizedDataFrame {
    ...
    pub fn mean_direct(&self, column_name: &str) -> Result<f64> {
        ...
        match column {
            Column::Float64(col) => {
                col.mean().ok_or(Error::EmptyDataFrame(format!("Column '{}' is empty", column_name)))
            },
            Column::Int64(col) => {
                col.mean().ok_or(Error::EmptyDataFrame(format!("Column '{}' is empty", column_name)))
            },
    ...
    pub fn max_direct(&self, column_name: &str) -> Result<f64> {
        ...
        match column {
            Column::Float64(col) => {
                col.max().ok_or(Error::EmptyDataFrame(format!("Column '{}' is empty", column_name)))
            },
            Column::Int64(col) => {
                col.max().map(|v| v as f64).ok_or(Error::EmptyDataFrame(format!("Column '{}' is empty", column_name)))
            },
            Column::String(_) => Err(Error::ColumnTypeMismatch {
                name: column_name.to_string(),
                expected: ColumnType::Float64,
    ...
    pub fn min_direct(&self, column_name: &str) -> Result<f64> {
        ...
        match column {
            Column::Float64(col) => {
                col.min().ok_or(Error::EmptyDataFrame(format!("Column '{}' is empty", column_name)))
            },
            Column::Int64(col) => {
                col.min().map(|v| v as f64).ok_or(Error::EmptyDataFrame(format!("Column '{}' is empty", column_name)))
            },
            Column::String(_) => Err(Error::ColumnTypeMismatch {
                name: column_name.to_string(),
                expected: ColumnType::Float64,
            ...
            Column::Float64(col) => {
                if col.null_mask.is_none() {
                    if col.data.is_empty() {
                        Err(Error::EmptyDataFrame(format!("Column '{}' is empty", column_name)))
                    } else {
...
mod tests {
    ...
    fn create_test_dataframe() -> OptimizedDataFrame {
        ...
        df.add_column("float_col".to_string(), Column::Float64(float_column)).unwrap();
        
        // Add Int64 column
        let int_data = vec![10, 20, 30, 40, 50];
        let int_column = Int64Column::new(int_data.clone());
        df.add_column("int_col".to_string(), Column::Int64(int_column)).unwrap();
    ...
    fn test_simd_vs_direct_consistency() {
        ...
        assert_eq!(df.sum_direct("float_col").unwrap(), df.sum_simd("float_col").unwrap());
        assert_eq!(df.mean_direct("float_col").unwrap(), df.mean_simd("float_col").unwrap());
        assert_eq!(df.max_direct("float_col").unwrap(), df.max_simd("float_col").unwrap());
        assert_eq!(df.min_direct("float_col").unwrap(), df.min_simd("float_col").unwrap());
        
        assert_eq!(df.sum_direct("int_col").unwrap(), df.sum_simd("int_col").unwrap());
        assert_eq!(df.mean_direct("int_col").unwrap(), df.mean_simd("int_col").unwrap());
        assert_eq!(df.max_direct("int_col").unwrap(), df.max_simd("int_col").unwrap());
        assert_eq!(df.min_direct("int_col").unwrap(), df.min_simd("int_col").unwrap());
    }
    ...
    fn test_simd_performance_with_large_dataset() {
        ...
        df.add_column("large_float".to_string(), Column::Float64(float_column)).unwrap();
        df.add_column("large_int".to_string(), Column::Int64(int_column)).unwrap();
        ...
        assert_eq!(df.sum_direct("large_float").unwrap(), df.sum_simd("large_float").unwrap());
        assert_eq!(df.mean_direct("large_float").unwrap(), df.mean_simd("large_float").unwrap());
        assert_eq!(df.max_direct("large_int").unwrap(), df.max_simd("large_int").unwrap());
        assert_eq!(df.min_direct("large_int").unwrap(), df.min_simd("large_int").unwrap());
    }
}