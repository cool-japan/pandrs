...
fn benchmark_enhanced_aggregations(c: &mut Criterion) {
    ...
    for &size in &[10_000, 100_000, 1_000_000] {
        ...
        let operations: &[(&str, Box<dyn Fn(&OptimizedDataFrame) -> f64>)] = &[
            ("sum", Box::new(|df: &OptimizedDataFrame| df.sum("trend_value").unwrap())),
            ("mean", Box::new(|df: &OptimizedDataFrame| df.mean("score").unwrap())),
            ("min", Box::new(|df: &OptimizedDataFrame| df.min("trend_value").unwrap())),
            ("max", Box::new(|df: &OptimizedDataFrame| df.max("trend_value").unwrap())),
        ];
