...
fn create_profiling_dataframe(size: usize, pattern: &str) -> Result<OptimizedDataFrame> {
    ...
    match pattern {
        ...
        _ => {
            return Err(pandrs::error::Error::InvalidValue(format!("Unknown pattern: {}", pattern)));
        }
    }
...
fn profile_aggregation_operations(c: &mut Criterion) {
    ...
    let aggregations = [
        ("standard_sum", Box::new(|df: &OptimizedDataFrame| df.sum("sequential_int").unwrap()) as Box<dyn Fn(&OptimizedDataFrame) -> f64>),
        ("standard_mean", Box::new(|df: &OptimizedDataFrame| df.mean("sequential_float").unwrap())),
    ];
...
fn profile_simd_operations(c: &mut Criterion) {
    ...
    for &size in &sizes {
        ...
        let patterns = [
            ("sequential", (0..size).map(|i| i as f64).collect::<Vec<f64>>()),
            ("random", {
                use rand::prelude::*;
                let mut rng = rand::rng();
                (0..size).map(|_| rng.random::<f64>() * 1000.0).collect()
            }),
            ("alternating", (0..size).map(|i| if i % 2 == 0 { i as f64 } else { -(i as f64) }).collect()),
        ];
        ...
        for (pattern_name, data) in patterns {
            ...
            let simd_ops = [
                ...
                ("parallel_sum", Box::new(|data: &[f64]| parallel::immediate::sum(data, None))),
                ...
                group.bench_with_input(
                    ...
                    |b, data| {
                        b.iter_custom(|iters| {
                            ...
                            let elements_per_sec = (size as f64 * iters as f64) / elapsed.as_secs_f64();
                            
                            if size == 100_000 {  // Only print for largest size to reduce noise
                                println!(
        ...
        group.bench_with_input(
            ...
            |b, df| {
                b.iter_custom(|iters| {
                    ...
                    let mb_per_sec = (file_size_estimate as f64 * iters as f64) / (1024.0 * 1024.0 * elapsed.as_secs_f64());
