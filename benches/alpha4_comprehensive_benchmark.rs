...
fn create_optimized_dataframe(size: usize) -> OptimizedDataFrame {
    ...
    df.add_column("id".to_string(), Column::Int64(Int64Column::new(ids))).unwrap();
    df.add_column("name".to_string(), Column::String(StringColumn::new(names))).unwrap();
    df.add_column("category".to_string(), Column::String(StringColumn::new(categories))).unwrap();
    df.add_column("value".to_string(), Column::Float64(Float64Column::new(values))).unwrap();
    df.add_column("flag".to_string(), Column::Boolean(BooleanColumn::new(flags))).unwrap();
...
fn bench_dataframe_creation(c: &mut Criterion) {
    ...
    for size in [1_000, 10_000, 100_000].iter() {
        group.bench_with_input(BenchmarkId::new("Traditional", size), size, |b, &size| {
            b.iter(|| {
                black_box(create_test_dataframe(size))
            })
        });
        
        group.bench_with_input(BenchmarkId::new("Optimized", size), size, |b, &size| {
            b.iter(|| {
                black_box(create_optimized_dataframe(size))
            })
        });
    }
...
fn bench_column_management(c: &mut Criterion) {
    ...
    for size in [1_000, 10_000, 100_000].iter() {
        ...
        group.bench_with_input(BenchmarkId::new("rename_columns", size), size, |b, &size| {
        ...
        group.bench_with_input(BenchmarkId::new("set_column_names", size), size, |b, &size| {
            let mut df = create_test_dataframe(size);
            let new_names = vec![
                "col1".to_string(), "col2".to_string(), 
                "col3".to_string(), "col4".to_string()
            ];
        ...
        group.bench_with_input(BenchmarkId::new("optimized_rename_columns", size), size, |b, &size| {
...
fn bench_enhanced_io(c: &mut Criterion) {
    ...
    for size in [1_000, 10_000, 50_000].iter() {
        ...
        group.bench_with_input(BenchmarkId::new("parquet_write_snappy", size), &df, |b, df| {
            b.iter(|| {
                let temp_file = NamedTempFile::new().unwrap();
                black_box(write_parquet(df, temp_file.path(), Some(ParquetCompression::Snappy)).unwrap())
            })
        });
        
        group.bench_with_input(BenchmarkId::new("parquet_write_gzip", size), &df, |b, df| {
            b.iter(|| {
                let temp_file = NamedTempFile::new().unwrap();
                black_box(write_parquet(df, temp_file.path(), Some(ParquetCompression::Gzip)).unwrap())
            })
        });
        ...
        group.bench_with_input(BenchmarkId::new("parquet_read", size), temp_file.path(), |b, path| {
            b.iter(|| {
                black_box(read_parquet(path).unwrap())
            })
        });
    }
...
fn bench_distributed_processing(c: &mut Criterion) {
    ...
    for concurrency in [1, 2, 4, 8].iter() {
        for size in [10_000, 100_000].iter() {
            ...
            group.bench_with_input(
                BenchmarkId::new(format!("dataframe_aggregation_{}threads", concurrency), size), 
                &(df, *concurrency), 
                |b, (df, concurrency)| {
                    b.iter(|| {
                        let mut context = DistributedContext::new_local(*concurrency).unwrap();
                        ...
                        let result = test_df.aggregate(&["category"], &[("value", "avg", "avg_value")]).unwrap();
                        black_box(result)
                    })
                }
            );
...
fn bench_memory_usage(c: &mut Criterion) {
    ...
    for size in [10_000, 100_000].iter() {
        ...
        group.bench_with_input(BenchmarkId::new("traditional_memory", size), size, |b, &size| {
            b.iter(|| {
        ...
        group.bench_with_input(BenchmarkId::new("optimized_memory", size), size, |b, &size| {
            b.iter(|| {
                let mut df = OptimizedDataFrame::new();
                let data: Vec<String> = (0..size).map(|i| format!("Category_{}", i % 10)).collect();
...
fn bench_aggregation_performance(c: &mut Criterion) {
    ...
    for size in [1_000, 10_000, 100_000].iter() {
        ...
        group.bench_with_input(BenchmarkId::new("traditional_groupby", size), &df, |b, df| {
            b.iter(|| {
...
fn bench_error_handling(c: &mut Criterion) {
    ...
    group.bench_function("invalid_column_names", |b| {
        b.iter(|| {
            let result = df.set_column_names(vec!["too".to_string(), "many".to_string(), "names".to_string(), "here".to_string(), "extra".to_string()]);
            black_box(result.is_err())
        })
    });
