...
│
├── src/
│   ├── dataframe/    - DataFrame関連の操作
│   │   ├── mod.rs    - 共通機能
│   │   ├── join.rs   - 結合操作
│   │   ├── apply.rs  - 関数適用とウィンドウ操作
│   │   └── transform.rs - 形状変換（melt, stack, unstack）
│   │
│   ├── optimized/    - 最適化実装（デフォルト）
│   │   ├── mod.rs    - モジュールエントリポイント
│   │   ├── dataframe.rs - 列指向DataFrame実装
│   │   ├── lazy.rs   - 遅延評価フレームワーク
│   │   └── operations.rs - 最適化された操作
│   │
│   ├── series/       - Series関連の実装
│   │   ├── mod.rs    - Series本体
│   │   └── na_series.rs - 欠損値対応
...
│   │   ├── mod.rs    - 列共通機能
│   │   ├── int64_column.rs - 整数列
│   │   ├── float64_column.rs - 浮動小数点列
│   │   ├── string_column.rs - 文字列列（最適化モード対応）
│   │   ├── boolean_column.rs - ブール列
│   │   ├── string_pool.rs - 文字列プール最適化
│   │   └── common.rs - 共通機能
│   │
│   ├── temporal/     - 時系列データ処理
│   │   ├── mod.rs       - 時系列本体
...
列指向ストレージと遅延評価を導入した最適化実装をデフォルトの実装として採用しました。パフォーマンステストの結果は以下の通りです：

### ベンチマーク結果 (1,000,000行のデータセット)

| 操作 | 従来実装（レガシー） | 現在のデフォルト実装 | 高速化率 |
|------|-------------------|------------------|----------|
...
> 注: 従来実装は参照用に `legacy-implementation` ブランチに保存されています。新しい実装が大幅なパフォーマンス向上を実現するため、最新のコードベースではデフォルトとして採用されています。

### pandas比較ベンチマーク (参考値)

| 操作 | pandas時間 | PandRSデフォルト実装 | pandasとの比較 |
|------|-----------|-------------------|--------------|
| 100万行DataFrame作成 | 216ms | 439ms | 0.49倍（51%遅い） |
| フィルタリング | 112ms | 186ms | 0.60倍（40%遅い） |
| グループ化集計 | 98ms | 192ms | 0.51倍（49%遅い） |
...
```python
# pandas -> PandRS
df_from_pd = pr.DataFrame.from_pandas(pd_df)

# PandRS -> pandas
pd_from_df = df.to_pandas()
```
...
Python BindingsにもDataFrameとLazyFrameの機能を実装し、pandasとの比較を行いました。結果は以下の通りです：

#### 基本性能比較

| データサイズ | 操作 | pandas | PandRS従来実装 | PandRSデフォルト実装 | 従来比 | pandas比 |
|------------|------|--------|--------------|----------------|--------|---------|
...
| 操作 | pandas → PandRS変換 | PandRS → pandas 変換 |
|------|-------------------|-----------------|
| 100,000行データ | 0.215秒 | 0.180秒 |

| 操作 | pandas | PandRS | LazyFrame実装 |
|------|--------|--------|--------------|
...
| データサイズ | PandRS→pandas (文字列プール前) | PandRS→pandas (文字列プール後) | 改善率 |
|------------|---------------------------|--------------------------|--------|
