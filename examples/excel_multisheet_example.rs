...
fn main() -> Result<()> {
    ...
    println!("API: get_sheet_info(\"{}\", \"Sales\").unwrap()", excel_file);
    ...
    println!("API: read_excel_sheets(\"{}\", None, true, 0, None).unwrap()", excel_file);
    ...
    println!("API: read_excel_with_info(\"{}\", Some(\"Sales\"), true, 0, None).unwrap()", excel_file);
...
fn create_sample_inventory_dataframe() -> Result<DataFrame> {
    ...
    let supplier_series = Series::new(suppliers.into_iter().map(|s| s.to_string()).collect(), Some("Supplier".to_string()))?;

    df.add_column("Product".to_string(), product_series)?;
    df.add_column("Stock_Level".to_string(), stock_series.to_string_series()?)?;
    df.add_column("Reorder_Point".to_string(), reorder_series.to_string_series()?)?;
...
fn create_sample_summary_dataframe() -> Result<DataFrame> {
    ...
    let metrics = vec!["Total Sales", "Average Order", "Top Product", "Total Revenue"];
    let values = vec!["100", "139.98", "Widget B", "2748.00"];

    let metric_series = Series::new(metrics.into_iter().map(|s| s.to_string()).collect(), Some("Metric".to_string()))?;
...
fn demonstrate_excel_patterns() {
    ...
    println!("   let data = read_excel_sheets(\"data.xlsx\", Some(&target_sheets), true, 0, None)?;");
