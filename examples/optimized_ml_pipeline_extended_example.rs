...
use pandrs::ml::{
    AdvancedPipeline, FeatureEngineeringStage, BinningStrategy, WindowOperation
};
...
fn main() -> Result<()> {
    ...
    df.add_float_column("price", vec![100.0, 102.0, 98.0, 105.0, 103.0, 107.0, 101.0, 110.0, 108.0, 112.0])?;
    df.add_float_column("volume", vec![1000.0, 1200.0, 800.0, 1500.0, 1100.0, 1300.0, 900.0, 1600.0, 1400.0, 1700.0])?;
    df.add_float_column("market_cap", vec![50000.0, 52000.0, 49000.0, 55000.0, 53000.0, 57000.0, 51000.0, 60000.0, 58000.0, 62000.0])?;
...
fn advanced_pipeline_example(df: &OptimizedDataFrame) -> Result<()> {
    ...
    println!("Shape: {} rows × {} columns", result.row_count(), result.column_count());
    ...
    for stage_exec in &summary.stage_details {
        ...
        println!("  Input rows: {}, Output rows: {}", stage_exec.input_rows, stage_exec.output_rows);
        ...
        .with_custom_transform(
            ...
            |df: &OptimizedDataFrame| -> Result<OptimizedDataFrame> {
                ...
                if let Ok(price_values) = df.get_float_column("price") {
                    let volatility: Vec<f64> = price_values.windows(2)
...
fn financial_analysis_pipeline(df: &OptimizedDataFrame) -> Result<()> {
        ...
        .with_custom_transform(
            ...
            |df: &OptimizedDataFrame| -> Result<OptimizedDataFrame> {
                ...
                if let Ok(prices) = df.get_float_column("price") {
                    let risk_scores: Vec<f64> = prices.windows(3)
                        .map(|window| {
                            if window.len() >= 3 {
    ...
    println!("Generated {} features from {} original features", 
             result.column_count(), df.column_count());
    ...
    for (i, stage_exec) in summary.stage_details.iter().enumerate() {
        println!("Stage {}: {} ({:?})", 
                 i + 1, stage_exec.stage_name, stage_exec.duration);
    }
