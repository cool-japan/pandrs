use pandrs::dataframe::{DataFrame, GroupWiseWindowExt, EnhancedDataFrameWindowExt};
...
fn main() -> Result<()> {
    ...
    df.add_column("symbol".to_string(), Series::new(symbols.iter().map(|s| s.to_string()).collect::<Vec<_>>(), Some("symbol".to_string()))?)?;
    ...
    let groupwise_rolling = df.rolling_by_group(vec!["symbol".to_string()], 3)
        .min_periods(2)
        .center(false)
    ...
    let groupwise_expanding = df.expanding_by_group(vec!["symbol".to_string()], 2)
        .columns(vec!["price".to_string(), "volume".to_string()]);
    ...
    let groupwise_ewm = df.ewm_by_group(vec!["symbol".to_string()])
        .span(4)
        .adjust(true)
    ...
    let groupwise_time_rolling = df.rolling_time_by_group(
    ...
    let multi_rolling = df.rolling_by_group(vec!["symbol".to_string()], 3)
        .min_periods(2);
    ...
    let custom_rolling = df.rolling_by_group(vec!["symbol".to_string()], 3)
        .columns(vec!["price".to_string()]);
    ...
    let combined_analysis = df.rolling_by_group(vec!["symbol".to_string()], 2)
        .columns(vec!["price".to_string()]);
