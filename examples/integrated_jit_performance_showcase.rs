...
fn create_comprehensive_dataset(size: usize) -> Result<DataFrame> {
    ...
    // Add all columns
    df.add_column("Open".to_string(), Series::new(opens, Some("Open".to_string()))?)?;
    df.add_column("High".to_string(), Series::new(highs, Some("High".to_string()))?)?;
    df.add_column("Low".to_string(), Series::new(lows, Some("Low".to_string()))?)?;
    df.add_column("Close".to_string(), Series::new(closes, Some("Close".to_string()))?)?;
    df.add_column("Volume".to_string(), Series::new(volumes, Some("Volume".to_string()))?)?;
    df.add_column("RSI".to_string(), Series::new(rsi, Some("RSI".to_string()))?)?;
    df.add_column("MACD".to_string(), Series::new(macd, Some("MACD".to_string()))?)?;
    df.add_column("Volatility".to_string(), Series::new(volatility, Some("Volatility".to_string()))?)?;

    Ok(df)
}

/// Benchmark integration between query engine and JIT window operations
fn benchmark_query_window_integration(df: &DataFrame, jit_context: &JitWindowContext) -> Result<()> {
    ...
    println!("     Filtered {} rows to {} rows in {:?}", df.row_count(), filtered.row_count(), query_time);
    ...
    for i in 1..=4 {
        ...
        let status = if i <= 2 { "Interpreted" } else { "JIT Compiled" };
        println!("     Execution {}: SMA calculated in {:?} ({})", i, window_time, status);
    }
    ...
    let jit_time = window_times[2..].iter().sum::<std::time::Duration>().as_nanos() / 2;
...
fn benchmark_rolling_operations(df: &DataFrame, jit_context: &JitWindowContext) -> Result<()> {
    ...
    for (op_name, window_size) in operations {
        ...
        println!("   | {:18} | {:8.2} ms | {:8.2} ms | {:8.2} ms | {:5.2}x |",
                op_name,
...
fn benchmark_technical_analysis_pipeline(df: &DataFrame, jit_context: &JitWindowContext) -> Result<()> {
    ...
    for i in 1..=3 {
        ...
        let status = if i <= 2 { "Interpreted/Compiling" } else { "JIT Optimized" };
...
fn benchmark_large_scale_processing(df: &DataFrame, jit_context: &JitWindowContext) -> Result<()> {
    ...
    for &window_size in &window_sizes {
        ...
        println!("     Standard: {:?} | JIT: {:?} | Speedup: {:.2}x | Throughput: {:.2}M rows/sec",
...
fn benchmark_memory_efficiency(jit_context: &JitWindowContext) -> Result<()> {
    ...
    println!("     Average JIT Speedup: {:.2}x", stats.average_speedup_ratio());
    
    println!("   Cache Management:");
    println!("     Functions in Cache: {}", jit_context.compiled_functions_count());
    println!("     Compilation Time: {:.2} ms", stats.compilation_time_ns as f64 / 1_000_000.0);
    println!("     Time Saved: {:.2} ms", stats.time_saved_ns as f64 / 1_000_000.0);
...
fn display_final_statistics(jit_context: &JitWindowContext) -> Result<()> {
    ...
    println!("├─────────────────────────────────────────────────────────────┤");
    println!("│ Total Compilations               │ {:25} │", stats.total_compilations());
    println!("│ Total JIT Executions             │ {:25} │", stats.jit_executions);
    println!("│ Cache Hit Ratio                  │ {:23.1}% │", stats.cache_hit_ratio * 100.0);
    println!("│ Average Speedup                  │ {:23.2}x │", stats.average_speedup_ratio());
    println!("│ Functions Cached                 │ {:25} │", jit_context.compiled_functions_count());
    println!("│ Total Compilation Time           │ {:20.2} ms │", stats.compilation_time_ns as f64 / 1_000_000.0);
    println!("│ Total Time Saved                 │ {:20.2} ms │", stats.time_saved_ns as f64 / 1_000_000.0);
    println!("└─────────────────────────────────────────────────────────────┘");
    ...
    println!("• Cache hit ratio of {:.1}% demonstrates effective function reuse", stats.cache_hit_ratio * 100.0);
