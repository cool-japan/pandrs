...
fn benchmark_aggregations(test_name: &str, df: &OptimizedDataFrame) -> Result<()> {
    ...
    println!("  • Sum:   {:.2}ms/op -> Result: {:.2}", sum_duration.as_secs_f64() * 1000.0 / num_iterations as f64, sum_result);
    println!("  • Mean:  {:.2}ms/op -> Result: {:.2}", mean_duration.as_secs_f64() * 1000.0 / num_iterations as f64, mean_result);
    println!("  • Max:   {:.2}ms/op -> Result: {:.2}", max_duration.as_secs_f64() * 1000.0 / num_iterations as f64, max_result);
    println!("  • Min:   {:.2}ms/op -> Result: {:.2}", min_duration.as_secs_f64() * 1000.0 / num_iterations as f64, min_result);
    println!("  • Count: {:.2}ms/op -> Result: {}", count_duration.as_secs_f64() * 1000.0 / num_iterations as f64, count_result);

    // Show theoretical old vs new comparison
    let avg_time = (sum_duration + mean_duration + max_duration + min_duration + count_duration).as_secs_f64() / 5.0;
    let estimated_old_time = avg_time * 4.0; // Conservative 4x slower estimate
    
    println!("Estimated Performance Improvement:");
    println!("  • New direct method: {:.2}ms average", avg_time * 1000.0 / num_iterations as f64);
    println!("  • Old conversion method: ~{:.2}ms average (estimated)", estimated_old_time * 1000.0 / num_iterations as f64);
    println!("  • Performance gain: ~{:.1}x faster", estimated_old_time / avg_time);
