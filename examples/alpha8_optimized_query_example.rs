...
use pandrs::dataframe::query::{QueryExt, QueryContext, Evaluator, OptimizedEvaluator};
...
fn create_sample_dataset() -> Result<DataFrame> {
    ...
    let active_series = Series::new(active.into_iter().map(|s| s.to_string()).collect(), Some("Active".to_string()))?;
...
fn create_large_sample_dataset(size: usize) -> Result<DataFrame> {
    ...
    let dept_values: Vec<String> = (0..size).map(|i| departments[i % departments.len()].to_string()).collect();
...
fn test_complex_query_optimizations(df: &DataFrame) -> Result<()> {
    ...
    let complex_query = "(Age > 25 && true) || (false && Score > 90) || Department == 'Engineering'";
    ...
    println!("  Results match: {}", result.row_count() == simplified_result.row_count());
