...
fn demo_column_management() -> std::result::Result<(), Box<dyn std::error::Error>> {
    ...
    println!("Created DataFrame with {} columns and {} rows", df.column_names().len(), df.row_count());
    ...
    println!("✨ rename_columns(): Renamed 5 columns in {:.2}ms", rename_duration.as_secs_f64() * 1000.0);
    ...
    println!("✨ set_column_names(): Set all 10 column names in {:.2}ms", set_names_duration.as_secs_f64() * 1000.0);
...
fn demo_string_pool_optimization() -> std::result::Result<(), Box<dyn std::error::Error>> {
    ...
    println!("Testing with {} strings, {}% unique (high duplication)", size, (unique_count * 100) / size);
    
    // Traditional approach (simulated)
    let start = Instant::now();
    let traditional_data: Vec<String> = (0..size).map(|i| format!("Category_{}", i % unique_count)).collect();
    ...
    println!("   Duration: {:.2}ms", traditional_duration.as_secs_f64() * 1000.0);
    ...
    opt_df.add_column("category".to_string(), Column::String(StringColumn::new(optimized_data)))?;
    ...
    println!("   Duration: {:.2}ms", optimized_duration.as_secs_f64() * 1000.0);
...
fn demo_enhanced_io() -> Result<(), Box<dyn std::error::Error>> {
    ...
    df.add_column("salary".to_string(), Column::Float64(Float64Column::new(salaries)))?;
    df.add_column("active".to_string(), Column::Boolean(BooleanColumn::new(active)))?;
    
    println!("Created test DataFrame with {} rows and {} columns", df.row_count(), df.column_count());
    ...
    println!("   Snappy: {:.2}ms, file size: {} bytes", snappy_write_duration.as_secs_f64() * 1000.0, snappy_size);
    println!("   Gzip: {:.2}ms, file size: {} bytes", gzip_write_duration.as_secs_f64() * 1000.0, gzip_size);
    ...
    println!("   Loaded {} rows, {} columns", loaded_df.row_count(), loaded_df.column_names().len());
    
    // Verify data integrity
    if loaded_df.row_count() == df.row_count() && loaded_df.column_names().len() == df.column_count() {
...
fn demo_memory_usage() -> std::result::Result<(), Box<dyn std::error::Error>> {
    ...
    println!("📊 Traditional approach:");
    println!("   Creation time: {:.2}ms", traditional_duration.as_secs_f64() * 1000.0);
    println!("   Estimated memory: {:.1}MB", traditional_memory);
    
    // Optimized approach
    let start = Instant::now();
    let mut opt_df = OptimizedDataFrame::new();
    let optimized_data: Vec<String> = (0..size).map(|i| categories[i % categories.len()].to_string()).collect();
    opt_df.add_column("department".to_string(), Column::String(StringColumn::new(optimized_data)))?;
    let optimized_duration = start.elapsed();
    let optimized_memory = traditional_memory * 0.4; // Estimated based on categorical optimization
    
    println!("⚡ Optimized approach:");
    println!("   Creation time: {:.2}ms", optimized_duration.as_secs_f64() * 1000.0);
...
fn demo_series_operations() -> std::result::Result<(), Box<dyn std::error::Error>> {
    ...
    println!("✨ set_name() operation: {:.4}ms", name_operation_duration.as_secs_f64() * 1000.0);
    
    // Fluent interface (with_name)
    let start = Instant::now();
    let _fluent_series = pandrs::series::Series::new(data.clone(), None)?
        .with_name("fluent_name".to_string());
    let fluent_duration = start.elapsed();
    
    println!("🌊 with_name() fluent interface: {:.2}ms", fluent_duration.as_secs_f64() * 1000.0);
    ...
    println!("🔄 to_string_series() conversion: {:.2}ms", conversion_duration.as_secs_f64() * 1000.0);
...
mod tests {
    ...
    fn test_performance_demo_runs() {
        ...
        df.add_column("test".to_string(), 
            pandrs::series::Series::new(vec![1, 2, 3], Some("test".to_string())).unwrap()).unwrap();
    ...
    fn test_string_pool_functionality() {
        let mut df = OptimizedDataFrame::new();
        let data = vec!["A".to_string(), "B".to_string(), "A".to_string(), "B".to_string()];
        df.add_column("category".to_string(), Column::String(StringColumn::new(data))).unwrap();
