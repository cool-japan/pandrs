...
fn main() -> Result<()> {
    ...
    let salaries = vec!["50000", "65000", "75000", "58000", "62000", "85000", "56000", "72000"];
    ...
    let exp_series = Series::new(experience.into_iter().map(|s| s.to_string()).collect(), Some("Experience".to_string()))?;
    ...
    let complex_query = df.query("(Age > 30 && Salary > 60000) || (Experience > 10 && Department != 'HR')")?;
    ...
    // Complex calculation
    let with_score = df.eval("(Salary / 1000) + (Experience * 2) - (Age * 0.5)", "EmployeeScore")?;
    ...
    context.add_function("salary_grade".to_string(), |args| {
        ...
        let salary = args[0];
        if salary >= 80000.0 { 5.0 }
        else if salary >= 70000.0 { 4.0 }
        else if salary >= 60000.0 { 3.0 }
        else if salary >= 50000.0 { 2.0 }
        else { 1.0 }
    });
