...
use pandrs::dataframe::advanced_indexing::{
    DatetimeIndex, PeriodIndex, PeriodFrequency, IntervalIndex, 
    IntervalClosed, CategoricalIndex, Index, IndexOperations
};
...
fn main() -> Result<()> {
    ...
    let dates = vec![
        "2024-01-01 09:00:00", "2024-01-02 09:00:00", "2024-01-03 09:00:00",
        "2024-01-04 09:00:00", "2024-01-05 09:00:00", "2024-01-08 09:00:00",
        "2024-01-09 09:00:00", "2024-01-10 09:00:00", "2024-01-11 09:00:00",
        "2024-01-12 09:00:00", "2024-01-15 09:00:00", "2024-01-16 09:00:00",
    ];
    
    let prices = vec![
        "100.5", "102.3", "101.8", "103.2", "104.1", "105.2",
        "106.8", "105.5", "107.3", "108.9", "110.2", "109.8"
    ];
    
    let volumes = vec![
        "15000", "18000", "16500", "22000", "19500", "21000",
        "24000", "17500", "25000", "23500", "26000", "20000"
    ];
    
    let sectors = vec![
        "Technology", "Technology", "Healthcare", "Technology", "Finance", "Healthcare",
        "Technology", "Finance", "Healthcare", "Technology", "Finance", "Healthcare"
    ];

    let date_series = Series::new(dates.into_iter().map(|s| s.to_string()).collect(), Some("Date".to_string()))?;
    let price_series = Series::new(prices.into_iter().map(|s| s.to_string()).collect(), Some("Price".to_string()))?;
    let volume_series = Series::new(volumes.into_iter().map(|s| s.to_string()).collect(), Some("Volume".to_string()))?;
    let sector_series = Series::new(sectors.into_iter().map(|s| s.to_string()).collect(), Some("Sector".to_string()))?;
    ...
    let (df_with_dt_index, dt_index) = df.set_datetime_index("Date", Some("DatetimeIdx".to_string()))?;
    ...
    println!("  Daily range (7 periods): {:?}", daily_range.to_string_vec());
    ...
    println!("  Hourly range (6 periods, 2h frequency): {:?}", hourly_range.to_string_vec());
    ...
    let (df_with_cat_index, mut cat_index) = df.set_categorical_index(
        "Sector",
        true,
        Some("SectorIndex".to_string()),
    )?;
    ...
    println!("  Categories after adding Energy and Utilities: {:?}", cat_index.categories);
    
    // Remove categories
    cat_index.remove_categories(vec!["Energy".to_string()])?;
    println!("  Categories after removing Energy: {:?}", cat_index.categories);
    ...
    let index1 = vec!["A".to_string(), "B".to_string(), "C".to_string(), "D".to_string()];
    let index2 = vec!["C".to_string(), "D".to_string(), "E".to_string(), "F".to_string()];
    ...
    let symmetric_diff_result = IndexOperations::symmetric_difference_string_indexes(&index1, &index2);
    ...
    println!("  Sorted values: {:?}", sorted_dt_index.to_string_vec()[..5].to_vec());
...
fn demonstrate_performance() -> Result<()> {
    ...
    let start = Instant::now();
    let large_cat_index = CategoricalIndex::new(large_categorical_data, Some("LargeCategorical".to_string()), false);
    let duration = start.elapsed();
    println!("CategoricalIndex creation ({} entries): {:?}", size, duration);
    ...
    let start = Instant::now();
    let _large_interval_index = IntervalIndex::cut(&large_numeric_data, 100, IntervalClosed::Right, Some("LargeIntervals".to_string()))?;
    let duration = start.elapsed();
    println!("IntervalIndex cut operation ({} values, 100 bins): {:?}", size, duration);
...
fn demonstrate_error_handling() -> Result<()> {
    ...
    match DatetimeIndex::date_range(
        test_dt,
        None,
        None,
        "d",
