...
#[cfg(feature = "sql")]
use pandrs::io::{
    read_sql, write_to_sql, execute_sql, read_sql_advanced, read_sql_table, write_sql_advanced,
    has_table, list_tables, get_table_schema, get_create_table_sql,
    SqlConnection, DatabaseConnection, SqlReadOptions, SqlWriteOptions,
    WriteMode, InsertMethod, SqlValue
};
...
fn main() -> Result<()> {
    ...
    println!("Created sample DataFrame with {} rows", sales_data.row_count());
    ...
    println!("}};");
    println!("let df = read_sql_advanced(\"SELECT * FROM large_table\", &conn, options).unwrap();");
    println!("Result: Memory-efficient reading of large datasets");

    // Demo: Parameterized queries
    println!("\n--- Parameterized Queries (Security) ---");
    println!("let params = vec![SqlValue::Real(500.0), SqlValue::Text(\"Widget A\".to_string())];");
    println!("let options = SqlReadOptions {{");
    ...
    println!("let options = SqlReadOptions {{");
    println!("    parse_dates: Some(vec![\"order_date\".to_string(), \"ship_date\".to_string()]),");
    println!("    ..Default::default()");
    println!("}};");
...
fn create_sample_dataframe() -> Result<DataFrame> {
    ...
    let customers = vec!["Acme Corp", "Beta LLC", "Gamma Inc", "Acme Corp", "Delta Co"];

    let date_series = Series::new(dates.into_iter().map(|s| s.to_string()).collect(), Some("Date".to_string()))?;
    let product_series = Series::new(products.into_iter().map(|s| s.to_string()).collect(), Some("Product".to_string()))?;
    ...
    let customer_series = Series::new(customers.into_iter().map(|s| s.to_string()).collect(), Some("Customer".to_string()))?;
