...
fn main() {
    ...
    for _ in 0..3 {
        ...
        serial_total += duration;
        println!(
            "Serial grouping and aggregation time (1 run): {duration:?}"
        );
        println!("Group count: {}", grouped_df.row_count());
    }
    let serial_time = serial_total / 3;
    println!(
        "Average serial grouping and aggregation time: {serial_time:?}"
    );
    ...
    for _ in 0..3 {
        ...
        parallel_total += duration;
        println!(
            "Parallel grouping and aggregation time (1 run): {duration:?}"
        );
        println!("Group count: {}", result_df.row_count());
    }
    let parallel_time = parallel_total / 3;
    println!(
        "Average parallel grouping and aggregation time: {parallel_time:?}"
    );
