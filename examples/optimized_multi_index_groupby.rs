...
fn main() -> Result<()> {
    ...
    let regions = vec!["East", "West", "East", "West", "East", "East", "West", "West", "East", "West"];
    ...
    let grouped_by_category = df.group_by(["category"])?.aggregate(vec![
        ...
        ("score".to_string(), AggregateOp::Mean, "score_mean".to_string()),
    ])?;
    ...
    let grouped_with_multi_index = df.group_by_with_options(["category", "region"], true)?
        .aggregate(vec![
            ("value".to_string(), AggregateOp::Sum, "value_sum".to_string()),
            ("score".to_string(), AggregateOp::Mean, "score_mean".to_string()),
        ])?;
    ...
    let parallel_grouped = df.group_by_with_options(["category", "region"], true)?
        .par_aggregate(vec![
            ("value".to_string(), AggregateOp::Sum, "value_sum".to_string()),
            ("score".to_string(), AggregateOp::Mean, "score_mean".to_string()),
            ("value".to_string(), AggregateOp::Max, "value_max".to_string()),
            ("score".to_string(), AggregateOp::Min, "score_min".to_string()),
        ])?;
    ...
    let grouped_flat = df.group_by_with_options(["category", "region"], false)?
        .aggregate(vec![
            ("value".to_string(), AggregateOp::Sum, "value_sum".to_string()),
            ("score".to_string(), AggregateOp::Mean, "score_mean".to_string()),
        ])?;
