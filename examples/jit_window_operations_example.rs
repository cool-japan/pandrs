...
fn create_financial_dataset(size: usize) -> Result<DataFrame> {
    ...
    let returns: Vec<String> = (0..size).map(|i| {
    ...
    df.add_column("Returns".to_string(), Series::new(returns, Some("Returns".to_string()))?)?;
...
fn create_sample_dataset() -> Result<DataFrame> {
    ...
    df.add_column("Returns".to_string(), Series::new(returns.into_iter().map(|s| s.to_string()).collect(), Some("Returns".to_string()))?)?;
    df.add_column("Volatility".to_string(), Series::new(volatility.into_iter().map(|s| s.to_string()).collect(), Some("Volatility".to_string()))?)?;
...
fn test_rolling_mean_performance(df: &DataFrame, jit_context: &JitWindowContext) -> Result<()> {
    ...
    let avg_standard = standard_times.iter().sum::<std::time::Duration>().as_nanos() as f64 / standard_times.len() as f64;
    let avg_jit = jit_times[2..].iter().sum::<std::time::Duration>().as_nanos() as f64 / (jit_times.len() - 2) as f64; // Only JIT executions
    let speedup = avg_standard / avg_jit;
    
    println!("   Performance Summary:");
    println!("     Average standard time: {:.2} μs", avg_standard / 1000.0);
...
fn test_rolling_std_performance(df: &DataFrame, jit_context: &JitWindowContext) -> Result<()> {
    ...
    if jit_times.len() > 2 {
        let avg_standard = standard_times.iter().sum::<std::time::Duration>().as_nanos() as f64 / standard_times.len() as f64;
        let avg_jit = jit_times[2..].iter().sum::<std::time::Duration>().as_nanos() as f64 / (jit_times.len() - 2) as f64;
        let speedup = avg_standard / avg_jit;
...
fn test_multiple_operations_jit(df: &DataFrame, jit_context: &JitWindowContext) -> Result<()> {
    ...
    let operations = vec![
        ("Rolling Mean (5)", Box::new(|df: &DataFrame, ctx: &JitWindowContext| df.jit_rolling(5, ctx).mean()) as Box<dyn Fn(&DataFrame, &JitWindowContext) -> Result<DataFrame>>),
        ("Rolling Sum (5)", Box::new(|df: &DataFrame, ctx: &JitWindowContext| df.jit_rolling(5, ctx).sum())),
        ("Rolling Min (5)", Box::new(|df: &DataFrame, ctx: &JitWindowContext| df.jit_rolling(5, ctx).min())),
        ("Rolling Max (5)", Box::new(|df: &DataFrame, ctx: &JitWindowContext| df.jit_rolling(5, ctx).max())),
    ];
...
fn demonstrate_jit_statistics(jit_context: &JitWindowContext) -> Result<()> {
    ...
    println!("     Expanding Compilations: {}", stats.expanding_compilations);
    ...
    println!("     Average Speedup: {:.2}x", stats.average_speedup_ratio());
    ...
    println!("\n   Cache Management:");
    println!("     Before clearing: {} functions", jit_context.compiled_functions_count());
...
fn comprehensive_performance_comparison(df: &DataFrame, jit_context: &JitWindowContext) -> Result<()> {
    ...
    for (operation_name, window_size) in test_cases {
        ...
        println!("   | {:15} | {:7.2} ms | {:7.2} ms | {:5.2}x |", 
                operation_name,
