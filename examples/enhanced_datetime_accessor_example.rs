...
fn main() -> Result<()> {
    ...
    let dt1 = NaiveDate::from_ymd_opt(2023, 1, 31).unwrap().and_hms_opt(14, 37, 23).unwrap(); // Jan 31 (month-end)
    let dt2 = NaiveDate::from_ymd_opt(2024, 2, 29).unwrap().and_hms_opt(9, 45, 15).unwrap();  // Leap day
    let dt3 = NaiveDate::from_ymd_opt(2023, 12, 23).unwrap().and_hms_opt(16, 22, 45).unwrap(); // Saturday
    let dt4 = NaiveDate::from_ymd_opt(2024, 6, 15).unwrap().and_hms_opt(11, 8, 30).unwrap();  // Mid-year
    ...
    let reference_date = NaiveDate::from_ymd_opt(2024, 1, 1).unwrap().and_hms_opt(0, 0, 0).unwrap();
    ...
    for (i, count) in bday_counts.values().iter().enumerate() {
        println!("  [{}]: {} -> {} business days", i, datetime_series.values()[i], count);
    }
    ...
    let month_ends = vec![
        NaiveDate::from_ymd_opt(2024, 1, 31).unwrap().and_hms_opt(16, 0, 0).unwrap(),
        NaiveDate::from_ymd_opt(2024, 2, 29).unwrap().and_hms_opt(16, 0, 0).unwrap(), // Leap year
        NaiveDate::from_ymd_opt(2024, 3, 31).unwrap().and_hms_opt(16, 0, 0).unwrap(),
        NaiveDate::from_ymd_opt(2024, 4, 30).unwrap().and_hms_opt(16, 0, 0).unwrap(),
    ];
    ...
    for i in 0..financial_series.len() {
        println!("  {}: Q{}, {} days in month, leap year: {}", 
    ...
    let hourly_data = vec![
        NaiveDate::from_ymd_opt(2024, 1, 15).unwrap().and_hms_opt(9, 7, 23).unwrap(),
        NaiveDate::from_ymd_opt(2024, 1, 15).unwrap().and_hms_opt(9, 23, 45).unwrap(),
        NaiveDate::from_ymd_opt(2024, 1, 15).unwrap().and_hms_opt(9, 38, 12).unwrap(),
        NaiveDate::from_ymd_opt(2024, 1, 15).unwrap().and_hms_opt(9, 52, 55).unwrap(),
    ];
    ...
    let week_start = NaiveDate::from_ymd_opt(2024, 1, 15).unwrap().and_hms_opt(9, 0, 0).unwrap(); // Monday
    ...
    for i in 0..week_series.len() {
        println!("  {}: Weekday {}, Weekend: {}, Business day: {}", 
    ...
    df.add_column("is_business_day".to_string(), dt_accessor.is_business_day()?)?;
    df.add_column("days_in_month".to_string(), dt_accessor.days_in_month()?)?;
    
    println!("Comprehensive datetime DataFrame created with {} rows and {} columns!", 
             datetime_series.len(), df.column_names().len());
    
    // Display some insights
    println!("\nDataFrame insights:");
    println!("- Contains {} leap year dates", dt_accessor.is_leap_year()?.values().iter().filter(|&&x| x).count());
    println!("- Contains {} business days", dt_accessor.is_business_day()?.values().iter().filter(|&&x| x).count());
