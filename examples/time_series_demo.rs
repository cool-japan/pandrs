...
use pandrs::time_series::core::{TimeSeriesBuilder, Frequency};
use pandrs::time_series::decomposition::{SeasonalDecomposition, DecompositionMethod};
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    for i in 0..100 {
        let timestamp = Utc.timestamp_opt(1640995200 + (i * 86400) as i64, 0).unwrap(); // Daily data
    ...
    let decomposer = SeasonalDecomposition::new(DecompositionMethod::Additive)
        .with_period(7); // Weekly seasonality
    ...
    println!("   Trend variance ratio: {:.3}", decomp_result.metrics.trend_variance_ratio);
    println!("   Seasonal variance ratio: {:.3}", decomp_result.metrics.seasonal_variance_ratio);
    ...
    println!("   Statistical features: mean={:.3}, std={:.3}", features.statistical.mean, features.statistical.std);
    println!("   Window features: {} moving averages", features.window.moving_averages.len());
    println!("   EMA features: {} alpha values", features.window.ema_features.len());
    println!("   Frequency features: dominant freq={:.3}", features.frequency.dominant_frequency);

    // 4. Demonstrate basic time series operations
    println!("\n⚙️  4. Time series operations...");
    
    // Rolling mean
    let rolling_mean = ts.rolling_mean(7)?;
    ...
    // Percentage change
    let pct_change = ts.pct_change(1)?;
    ...
    println!("   Mean statistical feature: {:.3}", features.statistical.mean);
