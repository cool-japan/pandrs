...
fn main() -> Result<()> {
    ...
    let values = vec![100.0, 102.0, 101.0, 103.0, 105.0, 104.0, 106.0, 108.0, 107.0, 109.0];
    let dates = vec![
    ...
    println!("Rolling Mean (window=5, min_periods=3): {:?}", rolling_min_periods.values());
    
    // Centered rolling window
    let rolling_centered = value_series.rolling(5)?.center(true).mean()?;
    println!("Centered Rolling Mean (window=5): {:?}", rolling_centered.values());
...
fn demonstrate_performance_scenarios() -> Result<()> {
    ...
    let large_data: Vec<f64> = (0..1000).map(|i| {
        100.0 + (i as f64 * 0.1) + ((i as f64 * 0.01).sin() * 10.0)
    }).collect();
