...
use pandrs::dataframe::indexing::{
    AdvancedIndexingExt, MultiLevelIndex, 
    IndexAligner, AlignmentStrategy, selectors
};
...
fn main() -> Result<()> {
    ...
    let employee_ids = vec!["E001", "E002", "E003", "E004", "E005", "E006", "E007", "E008"];
    let names = vec!["Alice", "Bob", "Charlie", "Diana", "Eve", "Frank", "Grace", "Henry"];
    let departments = vec!["IT", "HR", "IT", "Marketing", "IT", "Finance", "HR", "Finance"];
    let salaries = vec!["75000", "65000", "80000", "58000", "70000", "85000", "60000", "72000"];
    let ages = vec!["28", "35", "42", "26", "31", "45", "29", "38"];
    let experiences = vec!["3", "8", "15", "2", "5", "18", "4", "12"];

    let id_series = Series::new(employee_ids.into_iter().map(|s| s.to_string()).collect(), Some("ID".to_string()))?;
    let name_series = Series::new(names.into_iter().map(|s| s.to_string()).collect(), Some("Name".to_string()))?;
    let dept_series = Series::new(departments.into_iter().map(|s| s.to_string()).collect(), Some("Department".to_string()))?;
    let salary_series = Series::new(salaries.into_iter().map(|s| s.to_string()).collect(), Some("Salary".to_string()))?;
    let age_series = Series::new(ages.into_iter().map(|s| s.to_string()).collect(), Some("Age".to_string()))?;
    let exp_series = Series::new(experiences.into_iter().map(|s| s.to_string()).collect(), Some("Experience".to_string()))?;
    ...
    let selection = df.select()
        .rows(selectors::positions(vec![1, 3, 5]))
        .columns(selectors::cols(vec!["Name".to_string(), "Department".to_string(), "Salary".to_string()]))
        .select()?;
    println!("{:?}", selection);
    
    // Select with range and specific columns
    println!("\nSelect rows 2-5 and specific columns:");
    let range_selection = df.select()
        .rows(selectors::range(2, 6))
        .columns(selectors::cols(vec!["Name".to_string(), "Age".to_string(), "Experience".to_string()]))
        .select()?;
    ...
    let (indexed_df, multi_index) = df.set_multi_index(&["Department".to_string(), "Age".to_string()])?;
    ...
    let series1 = Series::new(vec!["A".to_string(), "B".to_string(), "C".to_string()], Some("Col1".to_string()))?;
    let series2 = Series::new(vec!["1".to_string(), "2".to_string(), "3".to_string(), "4".to_string(), "5".to_string()], Some("Col2".to_string()))?;
    ...
    let (aligned1_outer, aligned2_outer) = IndexAligner::align(&df1, &df2, AlignmentStrategy::Outer)?;
    ...
    let (aligned1_inner, aligned2_inner) = IndexAligner::align(&df1, &df2, AlignmentStrategy::Inner)?;
...
fn demonstrate_performance() -> Result<()> {
    ...
    let categories: Vec<String> = (1..=size).map(|i| {
        match i % 4 {
            0 => "A".to_string(),
            1 => "B".to_string(), 
            2 => "C".to_string(),
