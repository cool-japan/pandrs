...
use pandrs::dataframe::{DataFrame, EnhancedPlotExt, ColorScheme, PlotTheme, utils};
...
fn main() -> Result<()> {
    ...
    let numeric_cols = vec!["sales".to_string(), "advertising".to_string(), "temperature".to_string()];
    df.plot()
    ...
    let dark_config = dark_theme.apply_to_config(pandrs::dataframe::plotting::PlotConfig::default());
    let light_config = light_theme.apply_to_config(pandrs::dataframe::plotting::PlotConfig::default());
    
    println!("✅ Applied dark theme with color scheme: {:?}", dark_config.color_scheme);
    println!("✅ Applied light theme with color scheme: {:?}", light_config.color_scheme);
    ...
    println!("✅ Created comprehensive plotting report with {} files:", report_files.len());
    ...
    println!("✅ Quartiles: Q1={:.2}, Q2={:.2}, Q3={:.2}", quantiles[0], quantiles[1], quantiles[2]);
    ...
    println!("🌐 Open 'plots/interactive_dashboard.html' in a web browser for interactive visualization");
...
fn create_sample_data() -> Result<DataFrame> {
    ...
    // Sales data
    let sales = vec![
        "45000", "52000", "48000", "55000", "62000", "58000", 
        "51000", "47000", "49000", "56000", "61000", "59000",
        "53000", "46000", "50000", "57000", "63000", "60000"
    ];
    
    // Advertising spend
    let advertising = vec![
        "5000", "6200", "5800", "6500", "7200", "6800",
        "5900", "5400", "5700", "6600", "7100", "6900",
        "6300", "5300", "5800", "6700", "7300", "7000"
    ];
    ...
    // Product categories  
    let product = vec![
        "Electronics", "Clothing", "Home", "Sports", "Electronics", "Clothing",
    ...
    data.insert("sales".to_string(), sales.iter().map(|s| s.to_string()).collect());
    data.insert("advertising".to_string(), advertising.iter().map(|s| s.to_string()).collect());
    data.insert("temperature".to_string(), temperature.iter().map(|s| s.to_string()).collect());
    data.insert("region".to_string(), region.iter().map(|s| s.to_string()).collect());
    data.insert("product".to_string(), product.iter().map(|s| s.to_string()).collect());
    data.insert("month".to_string(), month.iter().map(|s| s.to_string()).collect());
...
mod tests {
    ...
    fn test_enhanced_plotting_integration() {
        let result = main();
        assert!(result.is_ok(), "Enhanced plotting integration example should complete successfully");
    }
    ...
    fn test_sample_data_creation() {
        ...
        assert!(df.column_names().len() >= 6, "Sample data should have at least 6 columns");
    }
