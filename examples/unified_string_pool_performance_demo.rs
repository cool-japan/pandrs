...
fn demo_string_deduplication() -> Result<()> {
    ...
    println!("Dataset: 1000 location records with {} unique cities", cities.len());
    println!("Results:");
    ...
    println!("  • Memory saved by deduplication: ~{:.1}%", stats.deduplication_ratio * 100.0);
    ...
    println!("  • Space reduction: {:.1}x", total_chars as f64 / actual_bytes as f64);
...
fn demo_memory_efficiency() -> Result<()> {
    ...
    let test_cases = vec![
        ...
        ("Medium Duplication", create_medium_duplication_dataset(1000)),
    ...
    for (name, data) in test_cases {
        ...
        println!("  • Deduplication savings: {:.1}%", stats.deduplication_ratio * 100.0);
        println!("  • Buffer utilization: {:.1}%", stats.memory_efficiency * 100.0);
    }
...
fn demo_performance_comparison() -> Result<()> {
    ...
    println!("  • Zero-copy column: {:.2}ms", zero_copy_creation_time.as_secs_f64() * 1000.0);
    println!("  • Traditional column: {:.2}ms", traditional_creation_time.as_secs_f64() * 1000.0);
    
    println!("\nOperation Performance (filtering):");
    println!("  • Zero-copy: {:.2}ms", zero_copy_ops_time.as_secs_f64() * 1000.0);
    println!("  • Traditional: {:.2}ms", traditional_ops_time.as_secs_f64() * 1000.0);
    ...
    println!("\nMemory Efficiency:");
    println!("  • Deduplication ratio: {:.1}%", zero_copy_stats.deduplication_ratio * 100.0);
    println!("  • Memory utilization: {:.1}%", zero_copy_stats.memory_efficiency * 100.0);
    println!("  • Unique vs total strings: {} / {}", zero_copy_stats.unique_strings, zero_copy_stats.total_strings);
...
fn demo_shared_pool_benefits() -> Result<()> {
    ...
    let column1 = SimpleZeroCopyStringColumn::with_shared_pool(column1_data, Arc::clone(&shared_pool))?;
    let column2 = SimpleZeroCopyStringColumn::with_shared_pool(column2_data, Arc::clone(&shared_pool))?;
    let column3 = SimpleZeroCopyStringColumn::with_shared_pool(column3_data, Arc::clone(&shared_pool))?;
    ...
    println!("  • Total strings across columns: {}", column1.len() + column2.len() + column3.len());
    println!("  • Total unique strings in pool: {}", pool_stats.unique_strings);
    println!("  • Cross-column deduplication: {:.1}%", pool_stats.deduplication_ratio * 100.0);
    println!("  • Memory sharing efficiency: {:.1}x", 
...
fn create_medium_duplication_dataset(size: usize) -> Vec<String> {
    ...
    (0..size).map(|i| format!("{}_{}", patterns[i % patterns.len()], i / patterns.len())).collect()
}

fn create_high_duplication_dataset(size: usize) -> Vec<String> {
    let base_strings = vec!["common", "frequent", "repeated"];
    (0..size).map(|i| base_strings[i % base_strings.len()].to_string()).collect()
}
