...
#[cfg(all(feature = "cuda", feature = "optimized"))]
use pandrs::gpu::{init_gpu, get_gpu_manager, GpuConfig};
#[cfg(all(feature = "cuda", feature = "optimized"))]
...
fn main() -> Result<()> {
    ...
    if device_status.available {
        println!("  Device Name: {}", device_status.device_name.unwrap_or_else(|| "Unknown".to_string()));
        println!("  CUDA Version: {}", device_status.cuda_version.unwrap_or_else(|| "Unknown".to_string()));
        println!("  Total Memory: {} MB", device_status.total_memory.unwrap_or(0) / (1024 * 1024));
        println!("  Free Memory: {} MB", device_status.free_memory.unwrap_or(0) / (1024 * 1024));
    } else {
...
fn financial_analysis_example(gpu_context: &GpuWindowContext) -> Result<()> {
    ...
    for (name, window_size) in operations {
        ...
        let start = Instant::now();
        let result = df.gpu_rolling(window_size, gpu_context)
            .columns(vec!["price".to_string(), "volume".to_string()])
            .mean()?;
        let time = start.elapsed();
        
        println!("  Calculation time: {:?}", time);
        println!("  Result shape: {} rows × {} columns", result.row_count(), result.column_count());
...
fn advanced_operations_example(gpu_context: &GpuWindowContext) -> Result<()> {
    ...
    for window_size in window_sizes {
        ...
        let start = Instant::now();
        let rolling_result = df.gpu_rolling(window_size, gpu_context)
        ...
        let start = Instant::now();
        let std_result = df.gpu_rolling(window_size, gpu_context)
            .min_periods(window_size / 2)
            .std(1)?;
...
fn performance_monitoring_example(gpu_context: &GpuWindowContext) -> Result<()> {
    ...
    println!("  Cache hit ratio: {:.2}%", jit_stats.cache_hit_ratio * 100.0);
    println!("  Average speedup: {:.2}ms", jit_stats.average_speedup_ratio());
    
    println!("\nGPU Acceleration Statistics:");
    println!("  GPU executions: {}", gpu_stats.gpu_executions);
    println!("  CPU fallbacks: {}", gpu_stats.cpu_fallbacks);
    println!("  GPU usage ratio: {:.2}%", gpu_stats.gpu_usage_ratio() * 100.0);
    println!("  Average GPU speedup: {:.2}x", gpu_stats.average_gpu_speedup);
    println!("  Memory allocation success: {:.2}%", gpu_stats.allocation_success_rate() * 100.0);
    println!("  Total GPU memory used: {:.2} MB", gpu_stats.total_gpu_memory_allocated as f64 / (1024.0 * 1024.0));
...
fn create_sample_dataframe(size: usize) -> Result<DataFrame> {
    ...
    df.add_column("volatility".to_string(), Series::new(
...
fn create_financial_dataset(size: usize) -> Result<DataFrame> {
    ...
    df.add_column("returns".to_string(), Series::new(
        returns.iter().map(|v| v.to_string()).collect(),
        Some("returns".to_string())
    )?)?;
    
    df.add_column("high".to_string(), Series::new(
        high_data.iter().map(|v| v.to_string()).collect(),
        Some("high".to_string())
    )?)?;
    
    df.add_column("low".to_string(), Series::new(
    ...
    for series_idx in 0..5 {
        ...
        for i in 0..size {
            ...
            let trend = match series_idx {
                ...
                2 => 0.0,    // No trend
                3 => if i % 100 < 50 { 0.002 } else { -0.002 }, // Cyclical
        ...
        df.add_column(column_name.clone(), Series::new(
...
#[cfg(all(feature = "cuda", feature = "optimized"))]
fn verify_results_similarity(cpu_result: &DataFrame, gpu_result: &DataFrame, column: &str) -> Result<()> {
    if cpu_result.row_count() != gpu_result.row_count() {
        println!("  Warning: Result row counts differ ({} vs {})", 
                 cpu_result.row_count(), gpu_result.row_count());
        return Ok(());
    }
    ...
    if differences == 0 {
        ...
        println!("  ⚠ Results differ significantly ({} differences, max: {:.2e})", differences, max_diff);
    }
...
fn display_sample_results(df: &DataFrame, num_rows: usize) -> Result<()> {
    ...
    let start_row = if row_count > num_rows { row_count - num_rows } else { 0 };
