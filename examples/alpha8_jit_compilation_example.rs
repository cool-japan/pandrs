...
use pandrs::dataframe::query::{QueryExt, QueryContext, JitEvaluator};
...
fn create_jit_test_dataset(size: usize) -> Result<DataFrame> {
    ...
    let factors: Vec<String> = (0..size).map(|i| ((i % 10) as f64 + 1.0).to_string()).collect();
    let scores: Vec<String> = (0..size).map(|i| (50.0 + (i as f64 % 50.0)).to_string()).collect();
    let categories = ["A", "B", "C", "D"];
    let cat_values: Vec<String> = (0..size).map(|i| categories[i % categories.len()].to_string()).collect();
...
fn create_small_sample_dataset() -> Result<DataFrame> {
    ...
    let id_series = Series::new(ids.into_iter().map(|s| s.to_string()).collect(), Some("ID".to_string()))?;
    let value_series = Series::new(values.into_iter().map(|s| s.to_string()).collect(), Some("Value".to_string()))?;
    let factor_series = Series::new(factors.into_iter().map(|s| s.to_string()).collect(), Some("Factor".to_string()))?;
    let score_series = Series::new(scores.into_iter().map(|s| s.to_string()).collect(), Some("Score".to_string()))?;
    let cat_series = Series::new(categories.into_iter().map(|s| s.to_string()).collect(), Some("Category".to_string()))?;
...
fn test_automatic_jit_compilation(df: &DataFrame) -> Result<()> {
    ...
    println!("    Compiled Expressions in Cache: {}", context.compiled_expressions_count());
...
fn demonstrate_jit_statistics(df: &DataFrame) -> Result<()> {
    ...
    println!("    JIT Executions: {}", stats.jit_executions);
    println!("    Native Executions: {}", stats.native_executions);
    println!("    Average Compilation Time: {:.2} μs", stats.average_compilation_time_ns() / 1000.0);
    println!("    JIT Speedup Ratio: {:.2}x", stats.jit_speedup_ratio());
    println!("    Compiled Expressions in Cache: {}", context.compiled_expressions_count());
    
    // Cache management
    println!("\n  Cache Management:");
    println!("    Cache size before clear: {}", context.compiled_expressions_count());
    context.clear_jit_cache();
    println!("    Cache size after clear: {}", context.compiled_expressions_count());
...
fn test_expression_types_jit(_df: &DataFrame) -> Result<()> {
    ...
    for (name, expr) in complex_tests {
        println!("    {}: {} -> JIT Compilable with optimizations", name, expr);
    }
...
fn test_jit_configuration(df: &DataFrame) -> Result<()> {
    ...
    for threshold in thresholds {
        ...
        println!("    Threshold {}: Compile after {} executions", threshold, threshold);
        ...
        let stats = context.jit_stats();
        println!("      Compilations: {}, JIT Executions: {}", 
...
fn demonstrate_real_world_jit(df: &DataFrame) -> Result<()> {
    ...
    for query in financial_queries {
        println!("    Query: {} -> Optimized for repeated financial analysis", query);
    ...
    let stats = context.jit_stats();
    println!("    Financial Scenario Stats: {} compilations, {:.2}x speedup", 
             stats.compilations, stats.jit_speedup_ratio());
    ...
    for query in scientific_queries {
        println!("    Query: {} -> Optimized for scientific computations", query);
    }
    ...
    for query in analysis_queries {
        println!("    Query: {} -> Optimized for repeated data transformations", query);
    }
